  ...
  subroutine inlet_ylo_3d(lo,hi,ngq,ngc,dx,Q,con,fd,rhs,aux,qin,dlo,dhi)
    integer, intent(in) :: lo(3), hi(3), ngq, ngc, dlo(3), dhi(3)
    double precision,intent(in   )::dx(3)
    double precision,intent(in   )::Q  (-ngq+lo(1):hi(1)+ngq,-ngq+lo(2):hi(2)+ngq,-ngq+lo(3):hi(3)+ngq,nprim)
    double precision,intent(in   )::con(-ngc+lo(1):hi(1)+ngc,-ngc+lo(2):hi(2)+ngc,-ngc+lo(3):hi(3)+ngc,ncons)
    ...
    double precision :: dpdn, dvdn
    double precision :: L(5+nspecies), Ltr(5+nspecies), lhs(ncons)
    double precision :: S_p, S_Y(nspecies), d_u, d_v, d_w, d_p, d_Y(nspecies)
    double precision :: hcal, cpWT, gam1, cs2

    double precision, dimension(lo(1):hi(1),lo(3):hi(3)) :: &
         drhodx, drhodz, dudx, dudz, dvdx, dvdz, dwdx, dwdz, dpdx, dpdz
    double precision, dimension(nspecies,lo(1):hi(1),lo(3):hi(3)) :: dYdx, dYdz
    ...
    call comp_trans_deriv_y_3d (j,lo,hi,ngq,Q,dxinv,dlo,dhi,dpdx,dpdz,dudx,dvdx,dvdz,dwdz)
    call comp_trans_deriv_y2_3d(j,lo,hi,ngq,Q,dxinv,dlo,dhi,drhodx,drhodz,dudz,dwdx,dYdx,dYdz)

    !$omp parallel do private(i,k,n,rho,u,v,w,T,Y,h,rhoE) &
    !$omp private(dpdn, dvdn, L, Ltr, lhs) &
    ...
    do k=lo(3),hi(3)
       do i=lo(1),hi(1)
          ...
          ! Simple 1D LODI 
          L(1) = (v-aux(ics,i,k))*0.5d0*(dpdn-rho*aux(ics,i,k)*dvdn)
          L(2) = -inlet_eta*Ru*rho*(T-qin(iTin,i,k)) & 
               / (Lydomain*aux(ics,i,k)*aux(iWbar,i,k))
          L(3) = inlet_eta*aux(ics,i,k)/Lydomain*(u-qin(iuin,i,k))
          L(4) = inlet_eta*aux(ics,i,k)/Lydomain*(w-qin(iwin,i,k))
          L(5) = inlet_eta*rho*cs2*(1.d0-Ma2_ylo)/(2.d0*Lydomain) &
               * (v-qin(ivin,i,k))
          L(6:) = inlet_eta*aux(ics,i,k)/Lydomain*(Y-qin(iYin1:,i,k))
          
          ! multi-D effects
          Ltr(2) = -u*(drhodx(i,k) - dpdx(i,k)/cs2) &
               &  - w*(drhodz(i,k) - dpdz(i,k)/cs2)
          Ltr(3) = -(u*dudx(i,k) + w*dudz(i,k) + dpdx(i,k)/rho)
          Ltr(4) = -(u*dwdx(i,k) + w*dwdz(i,k) + dpdz(i,k)/rho)
          Ltr(5) = -0.5*((u*dpdx(i,k)+w*dpdz(i,k))     &
               + rho*cs2*(dudx(i,k)+dwdz(i,k))     &
               + rho*aux(ics,i,k)*(u*dvdx(i,k)+w*dvdz(i,k)))
          Ltr(6:)= -(u*dYdx(:,i,k)+w*dYdz(:,i,k))
...
!   end subroutine inlet_yhi
  ...
  subroutine comp_trans_deriv_y2_3d(j,lo,hi,ngq,Q,dxinv,dlo,dhi,dddx,dddz,dudz,dwdx,dYdx,dYdz)
    integer, intent(in) :: j, lo(3), hi(3), ngq,dlo(3),dhi(3)
    double precision, intent(in) :: dxinv(3)
    double precision, intent(in) :: Q(-ngq+lo(1):hi(1)+ngq,-ngq+lo(2):hi(2)+ngq,-ngq+lo(3):hi(3)+ngq,nprim)
    double precision, intent(out):: dddx(         lo(1):hi(1),lo(3):hi(3))
    double precision, intent(out):: dddz(         lo(1):hi(1),lo(3):hi(3))
    double precision, intent(out):: dudz(         lo(1):hi(1),lo(3):hi(3))
    double precision, intent(out):: dwdx(         lo(1):hi(1),lo(3):hi(3))
    double precision, intent(out):: dYdx(nspecies,lo(1):hi(1),lo(3):hi(3))
    double precision, intent(out):: dYdz(nspecies,lo(1):hi(1),lo(3):hi(3))

    integer :: i, k, n, slo(3), shi(3)

    slo = dlo + stencil_ng
    shi = dhi - stencil_ng

    !$omp parallel private(i,k,n)
    ...
    do k=lo(3),hi(3)
       ...
       do i=slo(1),shi(1)
          dddx     (i,k) = dxinv(1)*first_deriv_8(q(i-4:i+4,j,k,qrho))
          dwdx     (i,k) = dxinv(1)*first_deriv_8(q(i-4:i+4,j,k,qw))
          do n=1,nspecies
             dYdx(n,i,k) = dxinv(1)*first_deriv_8(q(i-4:i+4,j,k,qy1+n-1))
          end do
       end do
       ...
       if (dlo(1) .eq. lo(1)) then
          i = lo(1)
          ! use completely right-biased stencil
          dddx     (i,k) = dxinv(1)*first_deriv_rb(q(i:i+3,j,k,qrho))
          dwdx     (i,k) = dxinv(1)*first_deriv_rb(q(i:i+3,j,k,qw))
          do n=1,nspecies                                 
             dYdx(n,i,k) = dxinv(1)*first_deriv_rb(q(i:i+3,j,k,qy1+n-1))          
          end do
          ...
          i = lo(1)+2
          ! use 4th-order stencil
          dddx     (i,k) = dxinv(1)*first_deriv_4(q(i-2:i+2,j,k,qrho))
          dwdx     (i,k) = dxinv(1)*first_deriv_4(q(i-2:i+2,j,k,qw))
          do n=1,nspecies                                  
             dYdx(n,i,k) = dxinv(1)*first_deriv_4(q(i-2:i+2,j,k,qy1+n-1))          
          end do

          i = lo(1)+3
          ! use 6th-order stencil
          dddx     (i,k) = dxinv(1)*first_deriv_6(q(i-3:i+3,j,k,qrho))
          dwdx     (i,k) = dxinv(1)*first_deriv_6(q(i-3:i+3,j,k,qw))
          do n=1,nspecies                                  
             dYdx(n,i,k) = dxinv(1)*first_deriv_6(q(i-3:i+3,j,k,qy1+n-1))          
          end do
       end if
       ...
       if (dhi(1) .eq. hi(1)) then
          ...
          i = hi(1)-1
          ! use 3rd-order slightly left-biased stencil
          dddx     (i,k) = dxinv(1)*first_deriv_l3(q(i-2:i+1,j,k,qrho))
          dwdx     (i,k) = dxinv(1)*first_deriv_l3(q(i-2:i+1,j,k,qw))
          do n=1,nspecies                                   
             dYdx(n,i,k) = dxinv(1)*first_deriv_l3(q(i-2:i+1,j,k,qy1+n-1)) 
          end do

          i = hi(1)
          ! use completely left-biased stencil
          dddx     (i,k) = dxinv(1)*first_deriv_lb(q(i-3:i,j,k,qrho))
          dwdx     (i,k) = dxinv(1)*first_deriv_lb(q(i-3:i,j,k,qw))
          do n=1,nspecies                                 
             dYdx(n,i,k) = dxinv(1)*first_deriv_lb(q(i-3:i,j,k,qy1+n-1))  
          end do
       end if
    ...
    do k=slo(3),shi(3)
       do i=lo(1),hi(1)
          dddz     (i,k) = dxinv(3)*first_deriv_8(q(i,j,k-4:k+4,qrho))
          dudz     (i,k) = dxinv(3)*first_deriv_8(q(i,j,k-4:k+4,qu))
          do n=1,nspecies
             dYdz(n,i,k) = dxinv(3)*first_deriv_8(q(i,j,k-4:k+4,qy1+n-1))
          end do
       end do
    end do
    !$omp end do nowait
    ...
    if (dlo(3) .eq. lo(3)) then
       ...
       !$omp do
       do i=lo(1),hi(1)
          dddz     (i,k) = dxinv(3)*first_deriv_rb(q(i,j,k:k+3,qrho))
          dudz     (i,k) = dxinv(3)*first_deriv_rb(q(i,j,k:k+3,qu))
          do n=1,nspecies
             dYdz(n,i,k) = dxinv(3)*first_deriv_rb(q(i,j,k:k+3,qy1+n-1))
          end do
       end do
       !$omp end do nowait
       ...
       !$omp do
       do i=lo(1),hi(1)
          dddz     (i,k) = dxinv(3)*first_deriv_r3(q(i,j,k-1:k+2,qrho))
          dudz     (i,k) = dxinv(3)*first_deriv_r3(q(i,j,k-1:k+2,qu))
          do n=1,nspecies
             dYdz(n,i,k) = dxinv(3)*first_deriv_r3(q(i,j,k-1:k+2,qy1+n-1))
          end do
       end do
       !$omp end do nowait
       ...
       !$omp do
       do i=lo(1),hi(1)
          dddz     (i,k) = dxinv(3)*first_deriv_4(q(i,j,k-2:k+2,qrho))
          dudz     (i,k) = dxinv(3)*first_deriv_4(q(i,j,k-2:k+2,qu))
          do n=1,nspecies
             dYdz(n,i,k) = dxinv(3)*first_deriv_4(q(i,j,k-2:k+2,qy1+n-1))
          end do
       end do
       !$omp end do nowait
       ...
       !$omp do
       do i=lo(1),hi(1)
          dddz     (i,k) = dxinv(3)*first_deriv_6(q(i,j,k-3:k+3,qrho))
          dudz     (i,k) = dxinv(3)*first_deriv_6(q(i,j,k-3:k+3,qu))
          do n=1,nspecies
             dYdz(n,i,k) = dxinv(3)*first_deriv_6(q(i,j,k-3:k+3,qy1+n-1))
          end do
       end do
       !$omp end do nowait
    end if
    ...
    if (dhi(3) .eq. hi(3)) then
       ...
       !$omp do
       do i=lo(1),hi(1)
          dddz     (i,k) = dxinv(3)*first_deriv_l3(q(i,j,k-2:k+1,qrho))
          dudz     (i,k) = dxinv(3)*first_deriv_l3(q(i,j,k-2:k+1,qu))
          do n=1,nspecies
             dYdz(n,i,k) = dxinv(3)*first_deriv_l3(q(i,j,k-2:k+1,qy1+n-1))
          end do
       end do
       !$omp end do nowait
       ...
       !$omp do
       do i=lo(1),hi(1)
          dddz     (i,k) = dxinv(3)*first_deriv_lb(q(i,j,k-3:k,qrho))
          dudz     (i,k) = dxinv(3)*first_deriv_lb(q(i,j,k-3:k,qu))
          do n=1,nspecies
             dYdz(n,i,k) = dxinv(3)*first_deriv_lb(q(i,j,k-3:k,qy1+n-1))
          end do
       end do
       !$omp end do nowait
    end if
  ...
  end subroutine comp_trans_deriv_y2_3d
...
! ----------------- 2D routine --------------------------
  ...
  subroutine inlet_ylo_2d(lo,hi,ngq,ngc,dx,Q,con,fd,rhs,aux,qin,dlo,dhi)
    integer, intent(in) :: lo(2), hi(2), ngq, ngc, dlo(2), dhi(2)
    double precision,intent(in   )::dx(2)
    double precision,intent(in   )::Q  (-ngq+lo(1):hi(1)+ngq,-ngq+lo(2):hi(2)+ngq,nprim)
    double precision,intent(in   )::con(-ngc+lo(1):hi(1)+ngc,-ngc+lo(2):hi(2)+ngc,ncons)
    ...
    double precision,intent(inout)::rhs(lo(1):hi(1),lo(2):hi(2),ncons)
    double precision,intent(in   )::aux(naux,lo(1):hi(1))
    double precision,intent(in   )::qin(nqin,lo(1):hi(1))
    ...
    double precision, dimension(lo(1):hi(1)) :: drhodx, dudx, dvdx, dpdx
    double precision, dimension(nspecies,lo(1):hi(1)) :: dYdx
    ...
    call comp_trans_deriv_y_2d (j,lo,hi,ngq,Q,dxinv,dlo,dhi,dpdx,dudx,dvdx)
    call comp_trans_deriv_y2_2d(j,lo,hi,ngq,Q,dxinv,dlo,dhi,drhodx,dYdx)

    !$omp parallel do private(i,n,rho,u,v,T,Y,h,rhoE) &
    !$omp private(dpdn, dvdn, L, Ltr, lhs) &
    ...
    do i=lo(1),hi(1)
       ...
       ! multi-D effects
       Ltr(2) = -u*(drhodx(i) - dpdx(i)/cs2) 
       Ltr(3) = -(u*dudx(i) + dpdx(i)/rho)
       Ltr(4) = -0.5*(u*dpdx(i)     &
               + rho*cs2*dudx(i)     &
               + rho*aux(ics,i)*u*dvdx(i))
       Ltr(5:)= -u*dYdx(:,i)
  ...
  subroutine comp_trans_deriv_y2_2d(j,lo,hi,ngq,Q,dxinv,dlo,dhi,dddx,dYdx)
    integer, intent(in) :: j, lo(2), hi(2), ngq,dlo(2),dhi(2)
    double precision, intent(in) :: dxinv(2)
    double precision, intent(in) :: Q(-ngq+lo(1):hi(1)+ngq,-ngq+lo(2):hi(2)+ngq,nprim)
    double precision, intent(out):: dddx(         lo(1):hi(1))
    double precision, intent(out):: dYdx(nspecies,lo(1):hi(1))

    integer :: i, n, slo(2), shi(2)
    ...
    do i=slo(1),shi(1)
       dddx     (i) = dxinv(1)*first_deriv_8(q(i-4:i+4,j,qrho))
       do n=1,nspecies
          dYdx(n,i) = dxinv(1)*first_deriv_8(q(i-4:i+4,j,qy1+n-1))
       end do
    end do
    !$omp end parallel do
    ...
    if (dlo(1) .eq. lo(1)) then
       i = lo(1)
       ! use completely right-biased stencil
       dddx     (i) = dxinv(1)*first_deriv_rb(q(i:i+3,j,qrho))
       do n=1,nspecies
          dYdx(n,i) = dxinv(1)*first_deriv_rb(q(i:i+3,j,qy1+n-1))          
       end do

       i = lo(1)+1
       ! use 3rd-order slightly right-biased stencil
       dddx     (i) = dxinv(1)*first_deriv_r3(q(i-1:i+2,j,qrho))
       do n=1,nspecies
          dYdx(n,i) = dxinv(1)*first_deriv_r3(q(i-1:i+2,j,qy1+n-1))          
       end do
       
       i = lo(1)+2
       ! use 4th-order stencil
       dddx     (i) = dxinv(1)*first_deriv_4(q(i-2:i+2,j,qrho))
       do n=1,nspecies
          dYdx(n,i) = dxinv(1)*first_deriv_4(q(i-2:i+2,j,qy1+n-1))          
       end do

       i = lo(1)+3
       ! use 6th-order stencil
       dddx     (i) = dxinv(1)*first_deriv_6(q(i-3:i+3,j,qrho))
       do n=1,nspecies
          dYdx(n,i) = dxinv(1)*first_deriv_6(q(i-3:i+3,j,qy1+n-1))          
       end do
    end if
    ...
    if (dhi(1) .eq. hi(1)) then
       ...
       i = hi(1)-1
       ! use 3rd-order slightly left-biased stencil
       dddx     (i) = dxinv(1)*first_deriv_l3(q(i-2:i+1,j,qrho))
       do n=1,nspecies
          dYdx(n,i) = dxinv(1)*first_deriv_l3(q(i-2:i+1,j,qy1+n-1)) 
       end do
       
       i = hi(1)
       ! use completely left-biased stencil
       dddx     (i) = dxinv(1)*first_deriv_lb(q(i-3:i,j,qrho))
       do n=1,nspecies
          dYdx(n,i) = dxinv(1)*first_deriv_lb(q(i-3:i,j,qy1+n-1))  
       end do
    end if

  end subroutine comp_trans_deriv_y2_2d
