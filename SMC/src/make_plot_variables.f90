...
contains
    ...
    do i = 1, nfabs(Q)
       ...
       if (dm .eq. 2) then
          if (icomp .eq. icomp_h) then
             call make_h_2d(pdp(:,:,1,icomp), qp, lo, hi, ngpd, ngq)                
          else if (icomp .eq. icomp_divu) then
             call make_divu_2d(pdp(:,:,1,icomp), qp, lo, hi, ngpd, ngq, dx, dlo,dhi)
          else if (icomp .eq. icomp_omegadot) then
             call make_omegadot_2d(pdp(:,:,1,icomp:nspecies-1), qp, lo, hi, ngpd, ngq)
          else
  ...
  subroutine make_h_2d(h, Q, lo, hi, ngh, ngq)
    integer, intent(in) :: lo(2), hi(2), ngh, ngq
    double precision, intent(out) :: h(-ngh+lo(1):hi(1)+ngh,-ngh+lo(2):hi(2)+ngh)
    double precision, intent(in ) :: Q(-ngq+lo(1):hi(1)+ngq,-ngq+lo(2):hi(2)+ngq,nprim)

    integer :: i,j, n, qyn, qhn

    !$omp parallel do private(i,j,n,qyn,qhn)
    do j=lo(2),hi(2)
       do i=lo(1),hi(1)
          h(i,j) = 0.d0
          do n=1, nspecies
  ...
  subroutine make_divu_2d(divu, Q, lo, hi, ngd, ngq, dx, dlo, dhi)
    ...
    integer, intent(in) :: lo(2), hi(2), ngd, ngq, dlo(2), dhi(2)
    double precision, intent(out) :: divu(-ngd+lo(1):hi(1)+ngd,-ngd+lo(2):hi(2)+ngd)
    double precision, intent(in ) ::    Q(-ngq+lo(1):hi(1)+ngq,-ngq+lo(2):hi(2)+ngq,nprim)
  ...
  subroutine make_omegadot_2d(odot, Q, lo, hi, ngo, ngq)
    use chemistry_module, only : molecular_weight

    integer, intent(in) :: lo(2), hi(2), ngo, ngq
    double precision, intent(out) :: odot(-ngo+lo(1):hi(1)+ngo,-ngo+lo(2):hi(2)+ngo,nspecies)
    double precision, intent(in ) ::    Q(-ngq+lo(1):hi(1)+ngq,-ngq+lo(2):hi(2)+ngq,nprim)
    ...
    !$omp parallel do private(i,j,n,iwrk,rwrk,wdot,Yt)
    do j=lo(2),hi(2)
