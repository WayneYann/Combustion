...
contains
  ...
  subroutine hypterm_3d (lo,hi,dx,cons,clo,chi,q,qlo,qhi,rhs_g,rlo,rhi,dlo_g,dhi_g,bclo,bchi)
    ...
    double precision :: tmpx(lo(1)-stencil_ng:hi(1)+stencil_ng), &
         tmpy(lo(1)-stencil_ng:hi(1)+stencil_ng,lo(2)-stencil_ng:hi(2)+stencil_ng), &
         tmpz(lo(1)-stencil_ng:hi(1)+stencil_ng,lo(2)-stencil_ng:hi(2)+stencil_ng,  &
         &    lo(3)-stencil_ng:hi(3)+stencil_ng)
  ...
  subroutine diffterm_1(q,qlo,qhi,rhs,rlo,rhi,mu,xi, &
    ...
    double precision :: tmpx(dlo(1):dhi(1)), tmpy( lo(1): hi(1),dlo(2):dhi(2)), &
         tmpz( lo(1): hi(1), lo(2): hi(2),dlo(3):dhi(3))
    double precision, dimension(dlo(1):dhi(1),dlo(2):dhi(2),dlo(3):dhi(3)) :: vsm
  ...
  subroutine diffterm_2(q,qlo,qhi,rhs,rlo,rhi,mu,xi,lam,dxy, &
    ...
    integer          :: i,j,k,n, qxn, qyn, qhn, iryn, qhias

    double precision :: mmtmp8(8,lo(1):hi(1)+1)
    double precision :: M8p(8,lo(1):hi(1)+1,lo(2):hi(2)+1,lo(3):hi(3)+1)
    double precision, dimension(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3)) :: sumdrY, sumrYv, gradp
             ...
             do k=dlo(3),dhi(3)
                do j=dlo(2),dhi(2)
                   do i=dlo(1),dhi(1)
                      ...
                      dxe(i,j,k,n) = dxy(i,j,k,n)*(q(i,j,k,qhn)-q(i,j,k,qhias))
                      dpe(i,j,k) = dpe(i,j,k) + dpy(i,j,k,n)*(q(i,j,k,qhn)-q(i,j,k,qhias))
                   end do
                end do
             end do
          end if
    ...
    if (reset_inactive_species) then
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             do i=slo(1),shi(1)
                rhs(i,j,k,iry_ias) = rhs(i,j,k,iry_ias) - sumdry(i,j,k)
             end do
          end do
       end do
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             ...
             if (reset_inactive_species) then
                ...
                rhs(i,j,k,iry_ias) = rhs(i,j,k,iry_ias) - &
                     foulo(1)*dx2inv(1) * rhstot
             ...
             if (reset_inactive_species) then
                ...
                rhs(i,j,k,iry_ias) = rhs(i,j,k,iry_ias) - sumdrytmp
             else
                do n=2,ncons
                   rhs(i,j,k,n) = rhs(i,j,k,n) + (Hcell(1,n) - Hcell(0,n)) * dx2inv(1)
                end do
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             ...
             if (reset_inactive_species) then
                ...
                rhs(i,j,k,iry_ias) =  rhs(i,j,k,iry_ias) - fouhi(1)*dx2inv(1) * rhstot
       ...
       do j=lo(2),hi(2)
          do i=lo(1),hi(1)
             ...
             if (reset_inactive_species) then
                ...
                rhs(i,j,k,iry_ias) = rhs(i,j,k,iry_ias) - &
                     foulo(3)*dx2inv(3) * rhstot

             else
             
                do n = 1, nspecies
                   rhs(i,j,k,iry1+n-1) =  rhs(i,j,k,iry1+n-1) - &
                        foulo(3)*dx2inv(3) * Ytmp(n)*rhstot
                end do
       ...
       do j=lo(2),hi(2)
          do i=lo(1),hi(1)
             ...
             if (reset_inactive_species) then
                ...
                rhs(i,j,k,iry_ias) = rhs(i,j,k,iry_ias) - fouhi(3)*dx2inv(3) * rhstot

             else

                do n = 1, nspecies
                   rhs(i,j,k,iry1+n-1) =  rhs(i,j,k,iry1+n-1) - &
                        fouhi(3)*dx2inv(3) * Ytmp(n)*rhstot
                end do
  ...
  subroutine chemterm_3d(lo,hi,q,qlo,qhi,up,uplo,uphi,upc,upclo,upchi,dt)
    ...
    double precision           :: upc(upclo(1):upchi(1),upclo(2):upchi(2),upclo(3):upchi(3),ncons)

    integer :: iwrk, i,j,k,n,np, iryn
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)
                ...
                do n=1, nspecies
                   iryn = iry1+n-1
                   upc(i,j,k,iryn) = dtinv*q(i,j,k,qrho)*(YTvode(n)-q(i,j,k,qy1+n-1))
                end do
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             ...
             do n=1, nspecies
                iryn = iry1+n-1
                do i=lo(1),hi(1)
                   upc(i,j,k,iryn) = wdot(i,n) * molecular_weight(n)
                   up (i,j,k,iryn) = upc(i,j,k,iryn)
                end do
             end do
