!
! This file was generated by expand.py based on kernels_s3d.f90.
!
...
!EXPAND             qx(i,j,k,idu) = dxinv(1)*first_deriv_4(q(i-2:i+2,j,k,qu))
             qx(i,j,k,idu) = dxinv(1) * &
                ( D4(1)*(q(i+1,j,k,qu)-q(i-1,j,k,qu)) &
                + D4(2)*(q(i+2,j,k,qu)-q(i-2,j,k,qu)) )
!EXPAND             qx(i,j,k,idv) = dxinv(1)*first_deriv_4(q(i-2:i+2,j,k,qv))
             qx(i,j,k,idv) = dxinv(1) * &
                ( D4(1)*(q(i+1,j,k,qv)-q(i-1,j,k,qv)) &
                + D4(2)*(q(i+2,j,k,qv)-q(i-2,j,k,qv)) )
!EXPAND             qx(i,j,k,idw) = dxinv(1)*first_deriv_4(q(i-2:i+2,j,k,qw))
             qx(i,j,k,idw) = dxinv(1) * &
...
!EXPAND             qx(i,j,k,idw) = dxinv(1)*first_deriv_4(q(i-2:i+2,j,k,qw))
    ...
    if (physbclo(2)) then
       do k=dlo(3),dhi(3)
          ...
          ! use 4th-order stencil
          do i=dlo(1),dhi(1)
!EXPAND             qy(i,j,k,idu) = dxinv(2)*first_deriv_4(q(i,j-2:j+2,k,qu))
             qy(i,j,k,idu) = dxinv(2) * &
                ( D4(1)*(q(i,j+1,k,qu)-q(i,j-1,k,qu)) &
                + D4(2)*(q(i,j+2,k,qu)-q(i,j-2,k,qu)) )
!EXPAND             qy(i,j,k,idv) = dxinv(2)*first_deriv_4(q(i,j-2:j+2,k,qv))
             qy(i,j,k,idv) = dxinv(2) * &
                ( D4(1)*(q(i,j+1,k,qv)-q(i,j-1,k,qv)) &
                + D4(2)*(q(i,j+2,k,qv)-q(i,j-2,k,qv)) )
!EXPAND             qy(i,j,k,idw) = dxinv(2)*first_deriv_4(q(i,j-2:j+2,k,qw))
             qy(i,j,k,idw) = dxinv(2) * &
...
!EXPAND             qy(i,j,k,idw) = dxinv(2)*first_deriv_4(q(i,j-2:j+2,k,qw))
    ...
    if (physbclo(3)) then
       ...
       do j=dlo(2),dhi(2)
          do i=dlo(1),dhi(1)
!EXPAND             qz(i,j,k,idu) = dxinv(3)*first_deriv_4(q(i,j,k-2:k+2,qu))
             qz(i,j,k,idu) = dxinv(3) * &
                ( D4(1)*(q(i,j,k+1,qu)-q(i,j,k-1,qu)) &
                + D4(2)*(q(i,j,k+2,qu)-q(i,j,k-2,qu)) )
!EXPAND             qz(i,j,k,idv) = dxinv(3)*first_deriv_4(q(i,j,k-2:k+2,qv))
             qz(i,j,k,idv) = dxinv(3) * &
                ( D4(1)*(q(i,j,k+1,qv)-q(i,j,k-1,qv)) &
                + D4(2)*(q(i,j,k+2,qv)-q(i,j,k-2,qv)) )
!EXPAND             qz(i,j,k,idw) = dxinv(3)*first_deriv_4(q(i,j,k-2:k+2,qw))
             qz(i,j,k,idw) = dxinv(3) * &
   ...
   do k=lo(3),hi(3)
       do j=lo(2),hi(2)
          ...
          if (physbclo(1)) then
             ...
             i = lo(1)+2
             ! use 4th-order stencil
!EXPAND             qx(i,j,k,idp) = dxinv(1) * first_deriv_4( q(i-2:i+2,j,k,qpres) )
             qx(i,j,k,idp) = dxinv(1) * &
                ( D4(1)*(q(i+1,j,k,qpres)-q(i-1,j,k,qpres)) &
                + D4(2)*(q(i+2,j,k,qpres)-q(i-2,j,k,qpres)) )
!EXPAND             qx(i,j,k,idT) = dxinv(1) * first_deriv_4( q(i-2:i+2,j,k,qtemp) )
             qx(i,j,k,idT) = dxinv(1) * &
                ( D4(1)*(q(i+1,j,k,qtemp)-q(i-1,j,k,qtemp)) &
                + D4(2)*(q(i+2,j,k,qtemp)-q(i-2,j,k,qtemp)) )

             i = lo(1)+3
             ! use 6th-order stencil
!EXPAND             qx(i,j,k,idp) = dxinv(1) * first_deriv_6( q(i-3:i+3,j,k,qpres) )
             qx(i,j,k,idp) = dxinv(1) * &
                ( D6(1)*(q(i+1,j,k,qpres)-q(i-1,j,k,qpres)) &
                + D6(2)*(q(i+2,j,k,qpres)-q(i-2,j,k,qpres)) &
                + D6(3)*(q(i+3,j,k,qpres)-q(i-3,j,k,qpres)) )
!EXPAND             qx(i,j,k,idT) = dxinv(1) * first_deriv_6( q(i-3:i+3,j,k,qtemp) )
             qx(i,j,k,idT) = dxinv(1) * &
                ( D6(1)*(q(i+1,j,k,qtemp)-q(i-1,j,k,qtemp)) &
                + D6(2)*(q(i+2,j,k,qtemp)-q(i-2,j,k,qtemp)) &
                + D6(3)*(q(i+3,j,k,qtemp)-q(i-3,j,k,qtemp)) )
          end if
...
!EXPAND             qx(i,j,k,idT) = dxinv(1) * first_deriv_4( q(i-2:i+2,j,k,qtemp) )
    ...
    do k=lo(3),hi(3)
       ...
       if (physbclo(2)) then
          ...
          ! use 4th-order stencil
          do i=lo(1),hi(1)
!EXPAND             qy(i,j,k,idp) = dxinv(2) * first_deriv_4( q(i,j-2:j+2,k,qpres) )
             qy(i,j,k,idp) = dxinv(2) * &
                ( D4(1)*(q(i,j+1,k,qpres)-q(i,j-1,k,qpres)) &
                + D4(2)*(q(i,j+2,k,qpres)-q(i,j-2,k,qpres)) )
!EXPAND             qy(i,j,k,idT) = dxinv(2) * first_deriv_4( q(i,j-2:j+2,k,qtemp) )
             qy(i,j,k,idT) = dxinv(2) * &
                ( D4(1)*(q(i,j+1,k,qtemp)-q(i,j-1,k,qtemp)) &
                + D4(2)*(q(i,j+2,k,qtemp)-q(i,j-2,k,qtemp)) )
          enddo

          j = lo(2)+3
          ! use 6th-order stencil
          do i=lo(1),hi(1)
!EXPAND             qy(i,j,k,idp) = dxinv(2) * first_deriv_6( q(i,j-3:j+3,k,qpres) )
             qy(i,j,k,idp) = dxinv(2) * &
                ( D6(1)*(q(i,j+1,k,qpres)-q(i,j-1,k,qpres)) &
                + D6(2)*(q(i,j+2,k,qpres)-q(i,j-2,k,qpres)) &
                + D6(3)*(q(i,j+3,k,qpres)-q(i,j-3,k,qpres)) )
!EXPAND             qy(i,j,k,idT) = dxinv(2) * first_deriv_6( q(i,j-3:j+3,k,qtemp) )
             qy(i,j,k,idT) = dxinv(2) * &
                ( D6(1)*(q(i,j+1,k,qtemp)-q(i,j-1,k,qtemp)) &
                + D6(2)*(q(i,j+2,k,qtemp)-q(i,j-2,k,qtemp)) &
                + D6(3)*(q(i,j+3,k,qtemp)-q(i,j-3,k,qtemp)) )
          enddo
       end if
...
!EXPAND             qy(i,j,k,idT) = dxinv(2) * first_deriv_4( q(i,j-2:j+2,k,qtemp) )
    ...
    if (physbclo(3)) then
       ...
       do j=lo(2),hi(2)
          do i=lo(1),hi(1)
!EXPAND             qz(i,j,k,idp) = dxinv(3) * first_deriv_4( q(i,j,k-2:k+2,qpres) )
             qz(i,j,k,idp) = dxinv(3) * &
                ( D4(1)*(q(i,j,k+1,qpres)-q(i,j,k-1,qpres)) &
                + D4(2)*(q(i,j,k+2,qpres)-q(i,j,k-2,qpres)) )
!EXPAND             qz(i,j,k,idT) = dxinv(3) * first_deriv_4( q(i,j,k-2:k+2,qtemp) )
             qz(i,j,k,idT) = dxinv(3) * &
                ( D4(1)*(q(i,j,k+1,qtemp)-q(i,j,k-1,qtemp)) &
                + D4(2)*(q(i,j,k+2,qtemp)-q(i,j,k-2,qtemp)) )
          enddo
       enddo
       ...
       do j=lo(2),hi(2)
          do i=lo(1),hi(1)
!EXPAND             qz(i,j,k,idp) = dxinv(3) * first_deriv_6( q(i,j,k-3:k+3,qpres) )
             qz(i,j,k,idp) = dxinv(3) * &
                ( D6(1)*(q(i,j,k+1,qpres)-q(i,j,k-1,qpres)) &
                + D6(2)*(q(i,j,k+2,qpres)-q(i,j,k-2,qpres)) &
                + D6(3)*(q(i,j,k+3,qpres)-q(i,j,k-3,qpres)) )
!EXPAND             qz(i,j,k,idT) = dxinv(3) * first_deriv_6( q(i,j,k-3:k+3,qtemp) )
             qz(i,j,k,idT) = dxinv(3) * &
                ( D6(1)*(q(i,j,k+1,qtemp)-q(i,j,k-1,qtemp)) &
                + D6(2)*(q(i,j,k+2,qtemp)-q(i,j,k-2,qtemp)) &
                + D6(3)*(q(i,j,k+3,qtemp)-q(i,j,k-3,qtemp)) )
          enddo
       enddo
    end if
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             ...
             if (physbclo(1)) then
                ...
                i = lo(1)+2
                ! use 4th-order stencil
!EXPAND                qx(i,j,k,qdxn) = dxinv(1) * first_deriv_4( q(i-2:i+2,j,k,qxn) )
                ...
                i = lo(1)+3
                ! use 6th-order stencil
!EXPAND                qx(i,j,k,qdxn) = dxinv(1) * first_deriv_6( q(i-3:i+3,j,k,qxn) )
                qx(i,j,k,qdxn) = dxinv(1) * &
...
!EXPAND                qx(i,j,k,qdxn) = dxinv(1) * first_deriv_4( q(i-2:i+2,j,k,qxn) )
       ...
       do k=lo(3),hi(3)
          ...
          if (physbclo(2)) then
             ...
             ! use 4th-order stencil
             do i=lo(1),hi(1)
!EXPAND                qy(i,j,k,qdxn) = dxinv(2) * first_deriv_4( q(i,j-2:j+2,k,qxn) )
             ...
             ! use 6th-order stencil
             do i=lo(1),hi(1)
!EXPAND                qy(i,j,k,qdxn) = dxinv(2) * first_deriv_6( q(i,j-3:j+3,k,qxn) )
                qy(i,j,k,qdxn) = dxinv(2) * &
...
!EXPAND                qy(i,j,k,qdxn) = dxinv(2) * first_deriv_4( q(i,j-2:j+2,k,qxn) )
       ...
       if (physbclo(3)) then
          ...
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)
!EXPAND                qz(i,j,k,qdxn) = dxinv(3) * first_deriv_4( q(i,j,k-2:k+2,qxn) )
          ...
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)
!EXPAND                qz(i,j,k,qdxn) = dxinv(3) * first_deriv_6( q(i,j,k-3:k+3,qxn) )
                qz(i,j,k,qdxn) = dxinv(3) * &
