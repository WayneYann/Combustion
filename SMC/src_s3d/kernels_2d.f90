...
contains
  ...
  subroutine chemterm_2d(lo,hi,q,qlo,qhi,up,uplo,uphi,upc,upclo,upchi,fd,flo,fhi,dt)
    ...
    integer,         intent(in):: lo(2),hi(2),qlo(2),qhi(2),uplo(2),uphi(2),&
         upclo(2),upchi(2),flo(2),fhi(2)
    double precision,intent(in):: q  (  qlo(1):  qhi(1),  qlo(2):  qhi(2),nprim)
    double precision           :: up ( uplo(1): uphi(1), uplo(2): uphi(2),ncons)
    double precision           :: upc(upclo(1):upchi(1),upclo(2):upchi(2),ncons)
    double precision           :: fd (  flo(1):  fhi(1),  flo(2):  fhi(2),ncons)

    integer :: iwrk, i,j,n,iryn
    double precision :: Yt(nspecies), wdot(nspecies), rwrk, p, diff(nspecies)
    ...
    do j=lo(2),hi(2)
       do i=lo(1),hi(1)
          ...
          do n=1, nspecies
             Yt(n) = q(i,j,qy1+n-1)
             diff(n) = fd(i,j,iry1+n-1)
          end do
          
          call ckpy(q(i,j,qrho), q(i,j,qtemp), Yt, iwrk, rwrk, p)

          call getrates(p, q(i,j,qtemp), Yt, diff, dt, iwrk, rwrk, wdot)
          
          do n=1, nspecies
             iryn = iry1+n-1
             upc(i,j,iryn) = wdot(n) * molecular_weight(n)
             up (i,j,iryn) = up (i,j,iryn) + upc(i,j,iryn)
          end do
