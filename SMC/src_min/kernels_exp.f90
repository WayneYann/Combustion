...
!EXPAND             mmtmp = matmul(vsp(i-4:i+3,j,k), M8)
             ...
             mmtmp(6) = vsp(i-3,j,k) * M8(2,6) &
                      ...
                      + vsp(i+2,j,k) * M8(7,6) &
                      + vsp(i+3,j,k) * M8(8,6)
             mmtmp(7) = vsp(i-2,j,k) * M8(3,7) &
                      + vsp(i-1,j,k) * M8(4,7) &
                      + vsp(i  ,j,k) * M8(5,7) &
                      + vsp(i+1,j,k) * M8(6,7) &
                      + vsp(i+2,j,k) * M8(7,7) &
                      + vsp(i+3,j,k) * M8(8,7)
             mmtmp(8) = vsp(i-1,j,k) * M8(4,8) &
                      + vsp(i  ,j,k) * M8(5,8) &
                      + vsp(i+1,j,k) * M8(6,8) &
                      + vsp(i+2,j,k) * M8(7,8) &
                      + vsp(i+3,j,k) * M8(8,8)
...
!EXPAND             mmtmp = matmul(mu(i-4:i+3,j,k), M8)
             ...
             mmtmp(6) = mu(i-3,j,k) * M8(2,6) &
                      ...
                      + mu(i+2,j,k) * M8(7,6) &
                      + mu(i+3,j,k) * M8(8,6)
             mmtmp(7) = mu(i-2,j,k) * M8(3,7) &
                      + mu(i-1,j,k) * M8(4,7) &
                      + mu(i  ,j,k) * M8(5,7) &
                      + mu(i+1,j,k) * M8(6,7) &
                      + mu(i+2,j,k) * M8(7,7) &
                      + mu(i+3,j,k) * M8(8,7)
             mmtmp(8) = mu(i-1,j,k) * M8(4,8) &
                      + mu(i  ,j,k) * M8(5,8) &
                      + mu(i+1,j,k) * M8(6,8) &
                      + mu(i+2,j,k) * M8(7,8) &
                      + mu(i+3,j,k) * M8(8,8)
...
!EXPAND             mmtmp = matmul(lam(i-4:i+3,j,k), M8)
             ...
             mmtmp(6) = lam(i-3,j,k) * M8(2,6) &
                      ...
                      + lam(i+2,j,k) * M8(7,6) &
                      + lam(i+3,j,k) * M8(8,6)
             mmtmp(7) = lam(i-2,j,k) * M8(3,7) &
                      + lam(i-1,j,k) * M8(4,7) &
                      + lam(i  ,j,k) * M8(5,7) &
                      + lam(i+1,j,k) * M8(6,7) &
                      + lam(i+2,j,k) * M8(7,7) &
                      + lam(i+3,j,k) * M8(8,7)
             mmtmp(8) = lam(i-1,j,k) * M8(4,8) &
                      + lam(i  ,j,k) * M8(5,8) &
                      + lam(i+1,j,k) * M8(6,8) &
                      + lam(i+2,j,k) * M8(7,8) &
                      + lam(i+3,j,k) * M8(8,8)
...
!EXPAND             mmtmp = matmul(M8, q(i-4:i+3,j,k,qpres))
             ...
             mmtmp(4) = M8T(1,4) * q(i-4,j,k,qpres) &
             ...
             mmtmp(5) = M8T(1,5) * q(i-4,j,k,qpres) &
             ...
             mmtmp(6) = M8T(2,6) * q(i-3,j,k,qpres) &
...
!EXPAND             mmtmp = matmul(mu(i,j-4:j+3,k), M8)
             ...
             mmtmp(6) = mu(i,j-3,k) * M8(2,6) &
                      ...
                      + mu(i,j+2,k) * M8(7,6) &
                      + mu(i,j+3,k) * M8(8,6)
             mmtmp(7) = mu(i,j-2,k) * M8(3,7) &
                      + mu(i,j-1,k) * M8(4,7) &
                      + mu(i,j  ,k) * M8(5,7) &
                      + mu(i,j+1,k) * M8(6,7) &
                      + mu(i,j+2,k) * M8(7,7) &
                      + mu(i,j+3,k) * M8(8,7)
             mmtmp(8) = mu(i,j-1,k) * M8(4,8) &
                      + mu(i,j  ,k) * M8(5,8) &
                      + mu(i,j+1,k) * M8(6,8) &
                      + mu(i,j+2,k) * M8(7,8) &
                      + mu(i,j+3,k) * M8(8,8)
...
!EXPAND             mmtmp = matmul(vsp(i,j-4:j+3,k), M8)
             ...
             mmtmp(6) = vsp(i,j-3,k) * M8(2,6) &
                      ...
                      + vsp(i,j+2,k) * M8(7,6) &
                      + vsp(i,j+3,k) * M8(8,6)
             mmtmp(7) = vsp(i,j-2,k) * M8(3,7) &
                      + vsp(i,j-1,k) * M8(4,7) &
                      + vsp(i,j  ,k) * M8(5,7) &
                      + vsp(i,j+1,k) * M8(6,7) &
                      + vsp(i,j+2,k) * M8(7,7) &
                      + vsp(i,j+3,k) * M8(8,7)
             mmtmp(8) = vsp(i,j-1,k) * M8(4,8) &
                      + vsp(i,j  ,k) * M8(5,8) &
                      + vsp(i,j+1,k) * M8(6,8) &
                      + vsp(i,j+2,k) * M8(7,8) &
                      + vsp(i,j+3,k) * M8(8,8)
...
!EXPAND             mmtmp = matmul(lam(i,j-4:j+3,k), M8)
             ...
             mmtmp(6) = lam(i,j-3,k) * M8(2,6) &
                      ...
                      + lam(i,j+2,k) * M8(7,6) &
                      + lam(i,j+3,k) * M8(8,6)
             mmtmp(7) = lam(i,j-2,k) * M8(3,7) &
                      + lam(i,j-1,k) * M8(4,7) &
                      + lam(i,j  ,k) * M8(5,7) &
                      + lam(i,j+1,k) * M8(6,7) &
                      + lam(i,j+2,k) * M8(7,7) &
                      + lam(i,j+3,k) * M8(8,7)
             mmtmp(8) = lam(i,j-1,k) * M8(4,8) &
                      + lam(i,j  ,k) * M8(5,8) &
                      + lam(i,j+1,k) * M8(6,8) &
                      + lam(i,j+2,k) * M8(7,8) &
                      + lam(i,j+3,k) * M8(8,8)
...
!EXPAND             mmtmp = matmul(M8, q(i,j-4:j+3,k,qpres))
             ...
             mmtmp(4) = M8T(1,4) * q(i,j-4,k,qpres) &
             ...
             mmtmp(5) = M8T(1,5) * q(i,j-4,k,qpres) &
             ...
             mmtmp(6) = M8T(2,6) * q(i,j-3,k,qpres) &
...
!EXPAND             mmtmp = matmul(mu(i,j,k-4:k+3), M8)
             ...
             mmtmp(6) = mu(i,j,k-3) * M8(2,6) &
                      ...
                      + mu(i,j,k+2) * M8(7,6) &
                      + mu(i,j,k+3) * M8(8,6)
             mmtmp(7) = mu(i,j,k-2) * M8(3,7) &
                      + mu(i,j,k-1) * M8(4,7) &
                      + mu(i,j,k  ) * M8(5,7) &
                      + mu(i,j,k+1) * M8(6,7) &
                      + mu(i,j,k+2) * M8(7,7) &
                      + mu(i,j,k+3) * M8(8,7)
             mmtmp(8) = mu(i,j,k-1) * M8(4,8) &
                      + mu(i,j,k  ) * M8(5,8) &
                      + mu(i,j,k+1) * M8(6,8) &
                      + mu(i,j,k+2) * M8(7,8) &
                      + mu(i,j,k+3) * M8(8,8)
...
!EXPAND             mmtmp = matmul(vsp(i,j,k-4:k+3), M8)
             ...
             mmtmp(6) = vsp(i,j,k-3) * M8(2,6) &
                      ...
                      + vsp(i,j,k+2) * M8(7,6) &
                      + vsp(i,j,k+3) * M8(8,6)
             mmtmp(7) = vsp(i,j,k-2) * M8(3,7) &
                      + vsp(i,j,k-1) * M8(4,7) &
                      + vsp(i,j,k  ) * M8(5,7) &
                      + vsp(i,j,k+1) * M8(6,7) &
                      + vsp(i,j,k+2) * M8(7,7) &
                      + vsp(i,j,k+3) * M8(8,7)
             mmtmp(8) = vsp(i,j,k-1) * M8(4,8) &
                      + vsp(i,j,k  ) * M8(5,8) &
                      + vsp(i,j,k+1) * M8(6,8) &
                      + vsp(i,j,k+2) * M8(7,8) &
                      + vsp(i,j,k+3) * M8(8,8)
...
!EXPAND             mmtmp = matmul(lam(i,j,k-4:k+3), M8)
             ...
             mmtmp(6) = lam(i,j,k-3) * M8(2,6) &
                      ...
                      + lam(i,j,k+2) * M8(7,6) &
                      + lam(i,j,k+3) * M8(8,6)
             mmtmp(7) = lam(i,j,k-2) * M8(3,7) &
                      + lam(i,j,k-1) * M8(4,7) &
                      + lam(i,j,k  ) * M8(5,7) &
                      + lam(i,j,k+1) * M8(6,7) &
                      + lam(i,j,k+2) * M8(7,7) &
                      + lam(i,j,k+3) * M8(8,7)
             mmtmp(8) = lam(i,j,k-1) * M8(4,8) &
                      + lam(i,j,k  ) * M8(5,8) &
                      + lam(i,j,k+1) * M8(6,8) &
                      + lam(i,j,k+2) * M8(7,8) &
                      + lam(i,j,k+3) * M8(8,8)
...
!EXPAND             mmtmp = matmul(M8, q(i,j,k-4:k+3,qpres))
             ...
             mmtmp(4) = M8T(1,4) * q(i,j,k-4,qpres) &
             ...
             mmtmp(5) = M8T(1,5) * q(i,j,k-4,qpres) &
             ...
             mmtmp(6) = M8T(2,6) * q(i,j,k-3,qpres) &
