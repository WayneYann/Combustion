...
{\tt SMC} {\footnote{People often ask what SMC means.  The name was
    ...
    Mach number combustion code called LMC, which also stands for
...
is the partial thermal conductivity, $T$ is the temperature, $h_k$ is
...
A mixture model in employed in {\tt SMC}.  The species diffusion
...
\item {\tt SMC\_MIN := t}\\
  ...
  minimalist version of {\tt SMC}.  This option determines whether we
  use the minimalist version or not.  Leaving this option empty will
  use the full version.
\item {\tt NDEBUG :=}\\
  This option determines whether we compile with support for debugging
  (usually also en- abling some runtime checks). Setting NDEBUG := t
  turns off debugging.
\item {\tt MPI := t}\\
