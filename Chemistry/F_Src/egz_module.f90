...
contains
  ...
  subroutine EGZINI(np_in, itls_in, iflag_in)
    integer, intent(in) :: np_in, itls_in, iflag_in
    logical, save :: first_call = .true.

    if (iflag_in.gt.5) then
       call bl_error("EGZ: iflag > 3 not supported")
    end if
    ...
    if (first_call .or. np.ne.np_in) then
       ...
       if (itls.ne.itls_in .or. iflag.ne.iflag_in) then
          call bl_error("EGZ: itls or iflag changed")
       end if
...
!-----------------------------------------------------------------------
    do n=1,ns
       ...
       case default
          call bl_error("EFZ: wrong value in nlin")
       end select
