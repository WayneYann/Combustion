...
C///////////////////////////////////////////////////////////////////////
C
C     CHEMKIN-III file mach.f
C
C     the public-domain (?) routines d1mach, i1mach and r1mach
C
C
C     The routines were obtained on 97/10/29 by searching for them
C     at the netlib site:
C
C        http://www.netlib.org/utk/misc/netlib_query.html
C
C     For many machines, the logic after the large comment section
C     produces the correct machine constants, so it is not necessary
C     to un-comment out anything.
C
C
C     97/10/29  Joseph Grcar
C
C        1. Created this file.
C
C        2. Tested on hp, sgi and sun.
C
C///////////////////////////////////////////////////////////////////////
...
C
C     IEEE MACHINES WITH 64-BIT INTS AND REALS, SUCH AS THE CRAY T3E
C
C      DATA IMACH( 1) /    5 /
C      DATA IMACH( 2) /    6 /
C      DATA IMACH( 3) /    7 /
...
C     IEEE MACHINES WITH 64-BIT INTS AND REALS, SUCH AS THE CRAY T3E
C
C      DATA SMALL(1) / 4503599627370496    /
C      DATA LARGE(1) / 9218868437227405311 /
C      DATA RIGHT(1) / 4368491638549381120 /
C      DATA DIVER(1) / 4372995238176751616 /
C      DATA LOG10(1) / 4599094494223104511 /, SC/987/
C
C     MACHINE CONSTANTS FOR AMDAHL MACHINES.
C
