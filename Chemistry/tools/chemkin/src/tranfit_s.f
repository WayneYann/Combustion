C  CVS $Revision: 1.1.1.1 $  created $Date: 2006/05/26 19:09:34 $
      SUBROUTINE TRANFT (LINCK, LINMC, LTRAN, LOUT, MAXTP,
...
C
      CALL MCFIT
     1(LINCK, LINMC, LTRAN, LOUT, KK, NFDIM, NO, NT, LENICK,
...
C
      SUBROUTINE MCFIT
     1(LINCK, LINMC, LTRAN, LOUT, KDIM, NFDIM, NO, NT, LENICK,
...
C
C*****precision > double
C      BOLTZ = RU / 6.0221367D23
C*****END precision > double
C*****precision > single
      BOLTZ = RU / 6.0221367E23
C*****END precision > single
      EPSIL  = 0.0
      FDTCGS = 1.0E-18
      FATCM  = 1.0E+08
...
C        Fit the conductivities and viscosities
     ...
     2                XETA, CV, ICKWRK, RCKWRK, COFLAM, COFETA)
C
...
C
         CCC = 0.0
C*****precision > double
C         CALL DPCOEF (NORD, CCC, COFLAM(1,K), FITWRK)
C*****END precision > double
C*****precision > single
         CALL PCOEF (NORD, CCC, COFLAM(1,K), FITWRK)
C*****END precision > single
C
C
C        Fit viscosity
...
C
         ...
         CCC = 0.0
C*****precision > double
C         CALL DPCOEF (NORD, CCC, COFETA(1,K), FITWRK)
C*****END precision > double
C*****precision > single
         CALL PCOEF (NORD, CCC, COFETA(1,K), FITWRK)
C*****END precision > single
C
1000  CONTINUE
C
...
C
C*****precision > double
C            CALL DPCOEF (NORD, CCC, COFD(1,K,J), FITWRK)
C*****END precision > double
C*****precision > single
            CALL PCOEF (NORD, CCC, COFD(1,K,J), FITWRK)
C*****END precision > single
 999     CONTINUE
1000  CONTINUE
C
...
C
C*****precision > double
C            CALL DPCOEF (NORD, CCC, COFTD(1,K,NLITE), FITWRK)
C*****END precision > double
C*****precision > single
            CALL PCOEF (NORD, CCC, COFTD(1,K,NLITE), FITWRK)
C*****END precision > single
C
 1000    CONTINUE
 1100 CONTINUE
...
C                                                                     C
C*****precision > double
C      DOUBLE PRECISION FUNCTION HORNER (N, A, X)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
...
C
C*****precision > double
C      DOUBLE PRECISION FUNCTION D12 (W1,W2,T,EPS1,EPS2,SIG1,SIG2,DST)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
...
C                                                                     C
C*****precision > double
C      DOUBLE PRECISION FUNCTION PARKER (T, EPS)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
...
C
C*****precision > double
C      DATA   PI32O2 /2.7841639984158539D+00/,
C     $       P2O4P2 /4.4674011002723397D+00/,
C     $       PI32   /5.5683279968317078D+00/
C*****END precision > double
C*****precision > single
      DATA   PI32O2 /2.7841639984158539E+00/,
     $       P2O4P2 /4.4674011002723397E+00/,
     $       PI32   /5.5683279968317078E+00/
C*****END precision > single
...
C---------------------------------------------------------------------C
C*****precision > double
C      DOUBLE PRECISION FUNCTION QINTRP (ARG, X, Y)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
...
C
C*****precision > double
C      DOUBLE PRECISION FUNCTION OMEG12 (N, TR, DR)
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
...
C                                                                    C
C*****precision > double
C      SUBROUTINE DP1VLU (L, NDER, X, YFIT, YP, A)
CC***BEGIN PROLOGUE  DP1VLU
CC***PURPOSE  Use the coefficients generated by DPOLFT to evaluate the
CC            polynomial fit of degree L, along with the first NDER of
CC            its derivatives, at a specified point.
CC***LIBRARY   SLATEC
CC***CATEGORY  K6
CC***TYPE      DOUBLE PRECISION (PVALUE-S, DP1VLU-D)
...
CC     Abstract
CC
CC     The subroutine  DP1VLU  uses the coefficients generated by DPOLFT
...
CC
CC     The parameters for  DP1VLU  are
CC
...
CC   890911  Removed unnecessary intrinsics.  (WRB)
...
CC***END PROLOGUE  DP1VLU
C      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C      INTEGER I,IC,ILO,IN,INP1,IUP,K1,K1I,K2,K3,K3P1,K3PN,K4,K4P1,K4PN,
C     * KC,L,LM1,LP1,MAXORD,N,NDER,NDO,NDP1,NORD
C      DOUBLE PRECISION A(*),CC,DIF,VAL,X,YFIT,YP(*)
C      CHARACTER*8 XERN1, XERN2
CC***FIRST EXECUTABLE STATEMENT  DP1VLU
...
CC
CC     end of SUBROUTINE DP1VLU
C      RETURN
C      END
CC                                                                    C
CC--------------------------------------------------------------------C
CC                                                                    C
C      SUBROUTINE DPCOEF (L, C, TC, A)
CC***BEGIN PROLOGUE  DPCOEF
CC***PURPOSE  Convert the DPOLFT coefficients to Taylor series form.
CC***LIBRARY   SLATEC
CC***CATEGORY  K1A1A2
CC***TYPE      DOUBLE PRECISION (PCOEF-S, DPCOEF-D)
...
CC     Abstract
CC
CC     DPOLFT computes the least squares polynomial fit of degree L as
CC     a sum of orthogonal polynomials.  DPCOEF changes this fit to its
...
CC
CC     The parameters for  DPCOEF  are
CC
CC     INPUT -- All TYPE REAL variables are DOUBLE PRECISION
...
CC                  degree polynomial fitted by  DPOLFT .
...
CC   891214  Prologue converted to Version 4.0 format.  (BAB)
CC   920501  Reformatted the REFERENCES section.  (WRB)
CC***END PROLOGUE  DPCOEF
CC
C      INTEGER I,L,LL,LLP1,LLP2,NEW,NR
C      DOUBLE PRECISION A(*),C,FAC,SAVE,TC(*)
CC***FIRST EXECUTABLE STATEMENT  DPCOEF
C      LL = ABS(L)
C      LLP1 = LL + 1
C      CALL DP1VLU (LL,LL,C,TC(1),TC(2),A)
...
C 3      TC(NEW) = SAVE
CC
CC     end of SUBROUTINE DPCOEF
C 4    RETURN
C      END
CC                                                                    C
CC--------------------------------------------------------------------C
CC                                                                    C
C      SUBROUTINE DPOLFT (N, X, Y, W, MAXDEG, NDEG, EPS, R, IERR, A)
CC***BEGIN PROLOGUE  DPOLFT
CC***PURPOSE  Fit discrete data in a least squares sense by polynomials
CC            in one variable.
CC***LIBRARY   SLATEC
CC***CATEGORY  K1A1A2
CC***TYPE      DOUBLE PRECISION (POLFIT-S, DPOLFT-D)
CC***KEYWORDS  CURVE FITTING, DATA FITTING,LEAST SQUARES,POLYNOMIAL FIT
CC***AUTHOR  Shampine, L. F., (SNLA)
CC           Davenport, S. M., (SNLA)
...
CC     subroutine  DPOLFT computes the weighted least-squares polynomial
...
CC     orthogonal polynomial form.  Subroutine  DP1VLU  may then be
CC     called to evaluate the fitted polynomials and any of their
CC     derivatives at any point.  The subroutine  DPCOEF  may be used to
CC     express the polynomial fits as powers of (X-C) for any specified
CC     point C.
CC
CC     The parameters for  DPOLFT  are
CC
CC     Input -- All TYPE REAL variables are DOUBLE PRECISION
...
CC                  W(1) is negative,  DPOLFT  will set all the weights
...
CC                  (1)  If  EPS  is input negative,  DPOLFT chooses the
...
CC                  (2)  If  EPS  is set to 0.,  DPOLFT  computes the
...
CC                       fitted polynomial.  DPOLFT  will increase the
...
CC                  before  DPOLFT  can proceed.  Valid input results
...
CC                  locations
CC
CC     Note - DPOLFT  calculates all fits of degrees up to and including
CC            NDEG .  Any or all of these fits can be evaluated or
CC            expressed as powers of (X-C) using  DP1VLU  and  DPCOEF
CC            after just one call to  DPOLFT .
CC
CC***REFERENCES  L. F. Shampine, S. M. Davenport and R. E. Huddleston,
CC                 Curve fitting by polynomials in one variable, Report
CC                 SLA-74-0270, Sandia Laboratories, June 1974.
CC***ROUTINES CALLED  DP1VLU, XERMSG
CC***REVISION HISTORY  (YYMMDD)
CC   740601  DATE WRITTEN
CC   890531  Changed all specific intrinsics to generic.  (WRB)
CC   891006  Cosmetic changes to prologue.  (WRB)
...
CC   891214  Prologue converted to Version 4.0 format.  (BAB)
CC   900315  CALLs to XERROR changed to CALLs to XERMSG.  (THJ)
CC   900911  Added variable YP to DOUBLE PRECISION declaration.  (WRB)
CC   920501  Reformatted the REFERENCES section.  (WRB)
CC   920527  Corrected erroneous statements in DESCRIPTION.  (WRB)
CC***END PROLOGUE  DPOLFT
C      INTEGER I,IDEGF,IERR,J,JP1,JPAS,K1,K1PJ,K2,K2PJ,K3,K3PI,K4,
C     * K4PI,K5,K5PI,KSIG,M,MAXDEG,MOP1,NDEG,NDER,NFAIL
C      DOUBLE PRECISION TEMD1,TEMD2
C      DOUBLE PRECISION A(*),DEGF,DEN,EPS,ETST,F,FCRIT,R(*),SIG,SIGJ,
C     * SIGJM1,SIGPAS,TEMP,X(*),XM,Y(*),YP,W(*),W1,W11
C      DOUBLE PRECISION CO(4,3)
C      SAVE CO
...
CC***FIRST EXECUTABLE STATEMENT  DPOLFT
C      M = ABS(N)
...
C      IF (EPS .LT. 0.0D0 .AND.  M .EQ. MOP1) GO TO 30
C      XM = M
C      ETST = EPS*EPS*XM
C      IF (W(1) .LT. 0.0D0) GO TO 2
C      DO 1 I = 1,M
C        IF (W(I) .LE. 0.0D0) GO TO 30
C 1      CONTINUE
C      GO TO 4
C 2    DO 3 I = 1,M
C 3      W(I) = 1.0D0
C 4    IF (EPS .GE. 0.0D0) GO TO 8
CC
CC DETERMINE SIGNIFICANCE LEVEL INDEX TO BE USED IN STATISTICAL TEST
CC FOR CHOOSING DEGREE OF POLYNOMIAL FIT
CC
C      IF (EPS .GT. (-.55D0)) GO TO 5
C      IDEGF = M - MAXDEG - 1
C      KSIG = 1
...
C      GO TO 8
C 5    KSIG = 1
C      IF (EPS .LT. (-.03D0)) KSIG = 2
C      IF (EPS .LT. (-.07D0)) KSIG = 3
CC
...
C        K4PI = K4 + I
C        K5PI = K5 + I
C        TEMD2 = TEMD1*A(K4PI)
C        R(I) = TEMD2
C        A(K5PI) = TEMD2 - R(I)
C 15     SIGJ = SIGJ + W(I)*((Y(I)-R(I)) - A(K5PI))**2
C      J = 0
CC
CC SEE IF POLYNOMIAL OF DEGREE 0 SATISFIES THE DEGREE SELECTION CRITERIA
CC
C      IF (EPS) 24,26,27
CC
CC INCREMENT DEGREE
...
C        K4PI = K4 + I
C        TEMD2 = A(K4PI)
C        TEMD1 = TEMD1 + X(I)*W(I)*TEMD2*TEMD2
C 18   CONTINUE
C      A(JP1) = TEMD1/W11
CC
...
C        K4PI = K4 + I
C        K5PI = K5 + I
C        TEMD2 = W(I)*((Y(I)-R(I))-A(K5PI))*A(K4PI)
...
C        K4PI = K4 + I
C        K5PI = K5 + I
C        TEMD2 = R(I) + A(K5PI) + TEMD1*A(K4PI)
C        R(I) = TEMD2
C        A(K5PI) = TEMD2 - R(I)
...
CC
C 23   IF (SIGJ .EQ. 0.0D0) GO TO 29
C      DEGF = M - J - 1
C      DEN = (CO(4,KSIG)*DEGF + 1.0D0)*DEGF
...
C
 ...
 15     SIGJ = SIGJ + W(I)*((Y(I)-R(I)) - A(K5PI))**2
      J = 0
C
C SEE IF POLYNOMIAL OF DEGREE 0 SATISFIES DEGREE SELECTION CRITERION
C
      IF (EPS) 24,26,27
C
C INCREMENT DEGREE
