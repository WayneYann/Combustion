...
C
      POK = .FALSE.
C*****precision > double
C      IF (INDEX(PREC, 'DOUB') .GT. 0) POK = .TRUE.
C*****END precision > double
C*****precision > single
      IF (INDEX(PREC, 'SING') .GT. 0) POK = .TRUE.
C*****END precision > single
C
...
C*****precision > double
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C      PARAMETER (ZERO=0.0D0, ONE=1.0D0, EIGHT=8.0D0)
C*****END precision > double
C*****precision > single
      IMPLICIT REAL (A-H, O-Z), INTEGER (I-N)
...
C*****precision > double
C      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C      PARAMETER (ONE=1.0D0, FDTCGS=1.0D-18, FATCM=1.0D8,
C     1           DIPMIN=1.0D-20, BOLTZ=1.38056D-16)
C*****END precision > double
C*****precision > single
      IMPLICIT REAL (A-H, O-Z), INTEGER (I-N)
      PARAMETER (ONE=1.0, FDTCGS=1.0E-18, FATCM=1.0E8,
...
C      CALL DGETRF (KK, KK, XL0000, KK, IPVT, INFO)
C      IF (INFO .NE. 0) THEN
C          WRITE (6,*) ' ERROR IN DGETRF, INFO = ', INFO
C          STOP
C      ENDIF
C      CALL DGETRI(KK, XL0000, KK, IPVT, WORK, KK, INFO)
C      IF (INFO .NE. 0) THEN
C          WRITE (6,*) ' ERROR IN DGETRI, INFO = ', INFO
C          STOP
C      ENDIF
...
C*****precision > single - lapack
      CALL SGETRF (KK, KK, XL0000, KK, IPVT, INFO)
      IF (INFO .NE. 0) THEN
          WRITE (6,*) ' ERROR IN SGETRF, INFO = ', INFO
          STOP
      ENDIF
      CALL SGETRI(KK, XL0000, KK, IPVT, WORK, KK, INFO)
      IF (INFO .NE. 0) THEN
          WRITE (6,*) ' ERROR IN SGETRI, INFO = ', INFO
          STOP
      ENDIF
...
C      PARAMETER (RU=8.314510D+07, PI= 3.1415926535897932D+00,
C     $                        PI32O2= 2.7841639984158539D+00,
...
C*****precision > single
      ...
      PARAMETER (RU=8.314510E+07, PI= 3.1415926535897932E+00,
     $                        PI32O2= 2.7841639984158539E+00,
...
C
C*****precision > double - lapack
C      CALL DGETRF (NN, NN, XL, KK3, IPVT, INFO)
C      IF (INFO .NE. 0) THEN
C          WRITE (6,*) ' ERROR IN DGETRF, INFO = ', INFO
C          STOP
C      ENDIF
C      CALL DGETRS('N', NN, 1, XL, KK3, IPVT, R, NN, INFO)
C      IF (INFO .NE. 0) THEN
...
C
C*****precision > single - lapack
      CALL SGETRF (NN, NN, XL, KK3, IPVT, INFO)
      IF (INFO .NE. 0) THEN
          WRITE (6,*) ' ERROR IN SGETRF, INFO = ', INFO
          STOP
      ENDIF
      CALL SGETRS('N', NN, 1, XL, KK3, IPVT, R, NN, INFO)
      IF (INFO .NE. 0) THEN
          WRITE (6,*) ' ERROR IN SGETRS, INFO = ', INFO
          STOP
      ENDIF
C*****END precision > single - lapack
C
