C  CVS $Revision: 1.1.1.1 $  created $Date: 2006/05/26 19:09:34 $
      SUBROUTINE TRANFT (LINMC, LINCK, LTRAN, MAXTP,
...
c     1  ' TRANFIT: CHEMKIN-III Transport property fitting code,',
...
C
      ...
      subroutine writeMangledFnName(name,uno)
      use String_Utility
      character*(*) name
      integer uno
      write(uno,'(A)') '#if defined(BL_FORT_USE_UPPERCASE)'
      write(uno,'(A)') '#define '//trim(name)//' '//StrUpCase(trim(name))
      write(uno,'(A)') '#elif defined(BL_FORT_USE_LOWERCASE)'
      write(uno,'(A)') '#define '//trim(name)//' '//StrLowCase(trim(name))
      write(uno,'(A)') '#elif defined(BL_FORT_USE_UNDERSCORE)'
      write(uno,'(A)') '#define '//trim(name)//' '//StrLowCase(trim(name))//'_'
      write(uno,'(A)') '#endif'
      end

      subroutine writeDoubleFn(name,val,uno)
      implicit none
      character*(*) name
      integer uno
     ...
     &           I-1,'] = ',vec(N,K),';'
      ...
      implicit none
      character*(*) name
      integer dim1, dim2, dim3, uno, K, N, L, I
...
C---------------------------------------------------------------------C
C                                                                     C
      SUBROUTINE MCCFORM (LINMC, LOUT, KERR, NO, KDIM, KK, NLITE,
     1                  LENIMC, LENRMC, PATM, WT, EPS, SIG, DIP, POL,
     2                  ZROT, NLIN, COFLAM, COFETA, COFD, KTDIF,
     3                  COFTD)
C
...
C
C     Linkfile version (should be 1.0)
c      WRITE (LINMC, CFMT, ERR=5003) FILVER
c      call writeCharacterFn('FILVER',FILVER,LINMC)

C     Tranfit program version
c      WRITE (LINMC, CFMT, ERR=5003) PRVERS
c      call writeCharacterFn('PRVERS',PRVERS,LINMC)
C
C     Precision
c      Write (LINMC, CFMT, ERR=5003) PREC
c      call writeCharacterFn('PREC',PREC,LINMC)
C
C     Whether tranfit successfully completed or not
c      WRITE (LINMC, LFMT, ERR=5001) KERR
C
      IF (KERR) THEN
         WRITE (LOUT, '(//A,/A)')
     1   ' WARNING...THERE IS AN ERROR IN THE TRANSPORT LINKFILE.'
...
C
C     Work space sizes, fixed parameters
c      WRITE (LINMC, IFMT, ERR=5001) LENIMC, LENRMC, NO, KK, NLITE
      call writeIntFn('LENIMC',LENIMC,LINMC)
      call writeIntFn('LENRMC',LENRMC,LINMC)
      call writeIntFn('NO',NO,LINMC)
      call writeIntFn('KK',KK,LINMC)
      call writeIntFn('NLITE',NLITE,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002) PATM
      call writeDoubleFn('PATM',PATM,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002) (WT(K), K = 1, KK)
      call writeDoubleArrayFn('WT',WT,KK,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002) (EPS(K), K = 1, KK)
      call writeDoubleArrayFn('EPS',EPS,KK,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002) (SIG(K), K = 1, KK)
      call writeDoubleArrayFn('SIG',SIG,KK,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002) (DIP(K), K = 1, KK)
      call writeDoubleArrayFn('DIP',DIP,KK,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002) (POL(K), K = 1, KK)
      call writeDoubleArrayFn('POL',POL,KK,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002) (ZROT(K), K = 1, KK)
      call writeDoubleArrayFn('ZROT',ZROT,KK,LINMC)

c      WRITE (LINMC, IFMT, ERR=5001) (NLIN(K), K = 1, KK)
      call writeIntArrayFn('NLIN',NLIN,KK,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002) ((COFLAM(N,K), N=1,NO), K=1,KK)
      call write2DDoubleArrayFn('COFLAM',COFLAM,NO,KK,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002) ((COFETA(N,K), N=1,NO), K=1,KK)
      call write2DDoubleArrayFn('COFETA',COFETA,NO,KK,LINMC)

c      WRITE (LINMC, RFMT, ERR=5002)
c     1   (((COFD(N,J,K), N=1,NO), J=1,KK), K=1,KK)
...
C                                                                     C
C*****precision > double
      DOUBLE PRECISION FUNCTION HORNER (N, A, X)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
...
C
C*****precision > double
      DOUBLE PRECISION FUNCTION D12 (W1,W2,T,EPS1,EPS2,SIG1,SIG2,DST)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
...
C                                                                     C
C*****precision > double
      DOUBLE PRECISION FUNCTION PARKER (T, EPS)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
...
C---------------------------------------------------------------------C
C*****precision > double
      DOUBLE PRECISION FUNCTION QINTRP (ARG, X, Y)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
C*****precision > single
