	    ...
	    double * restrict wdot)
{
  ...
  double * restrict py = y;
  for (int n=0; n<9; n++) {
    for (int i=0; i<(*np); i++) {
      *pc++ = 1.0e6 * rho[i] * (*py++) * imw[n];
    }
  }
...
void vproductionRate(int npt, double * restrict wdot, double * restrict c, double * restrict T)
{
  double k_f[21][npt], Kc[21][npt], mixture[npt], g_RT[9*npt];
  ...
  for (int i=0; i<npt; i++) {
    k_f[0][i] = 1.e-06 * 3.547e+15*exp(-0.406*tc[i]-8352.8934356925419706*invT[i]);
    k_f[1][i] = 1.e-06 * 50800.*exp(2.67*tc[i]-3165.2328279116868543*invT[i]);
    k_f[2][i] = 1.e-06 * 2.16e+08*exp(1.51*tc[i]-1726.0331637101885462*invT[i]);
    k_f[3][i] = 1.e-06 * 2.97e+06*exp(2.02*tc[i]-6743.1033217832446098*invT[i]);
    k_f[4][i] = 1.e-06 * 4.577e+19*exp(-1.4*tc[i]-52525.75557669664704*invT[i]);
    k_f[5][i] = 1.e-12 * 6.165e+15*exp(-0.5*tc[i]);
    k_f[6][i] = 1.e-12 * 4.714e+18*exp(-1.*tc[i]);
    k_f[7][i] = 1.e-12 * 3.8e+22*exp(-2.*tc[i]);
    k_f[8][i] = 1.e-06 * 1.475e+12*exp(0.6*tc[i]);
    k_f[9][i] = 1.e-06 * 1.66e+13*exp(-414.14731595728432012*invT[i]);
    k_f[10][i] = 1.e-06 * 7.079e+13*exp(-148.44891641239232172*invT[i]);
    k_f[11][i] = 1.e-06 * 3.25e+13;
    k_f[12][i] = 1.e-06 * 2.89e+13*exp(+250.09868290494571852*invT[i]);
    k_f[13][i] = 1.e-06 * 4.2e+14*exp(-6029.5420896721516328*invT[i]);
    k_f[14][i] = 1.e-06 * 1.3e+11*exp(+819.89091359562974048*invT[i]);
    k_f[15][i] = 1. * 2.951e+14*exp(-24370.783124922574643*invT[i]);
    k_f[16][i] = 1.e-06 * 2.41e+13*exp(-1997.7701632447369775*invT[i]);
    k_f[17][i] = 1.e-06 * 4.82e+13*exp(-4000.5724931475210724*invT[i]);
    k_f[18][i] = 1.e-06 * 9.55e+06*exp(2.*tc[i]-1997.7701632447369775*invT[i]);
    k_f[19][i] = 1.e-06 * 1.e+12;
    k_f[20][i] = 1.e-06 * 5.8e+14*exp(-4809.2416750957054319*invT[i]);
  }
  ...
  for (int i=0; i<npt; i++) {
    ...
    Kc[0][i] = exp((g_RT[3*npt+i] + g_RT[1*npt+i]) - (g_RT[4*npt+i] + g_RT[5*npt+i]));
    Kc[1][i] = exp((g_RT[4*npt+i] + g_RT[i]) - (g_RT[3*npt+i] + g_RT[5*npt+i]));
    Kc[2][i] = exp((g_RT[i] + g_RT[5*npt+i]) - (g_RT[2*npt+i] + g_RT[3*npt+i]));
    Kc[3][i] = exp((g_RT[4*npt+i] + g_RT[2*npt+i]) - (g_RT[5*npt+i] + g_RT[5*npt+i]));
    Kc[4][i] = refC * exp((g_RT[i]) - (g_RT[3*npt+i] + g_RT[3*npt+i]));
    Kc[5][i] = 1.0 / (refC) * exp((g_RT[4*npt+i] + g_RT[4*npt+i]) - (g_RT[1*npt+i]));
    Kc[6][i] = 1.0 / (refC) * exp((g_RT[4*npt+i] + g_RT[3*npt+i]) - (g_RT[5*npt+i]));
    Kc[7][i] = 1.0 / (refC) * exp((g_RT[3*npt+i] + g_RT[5*npt+i]) - (g_RT[2*npt+i]));
    Kc[8][i] = 1.0 / (refC) * exp((g_RT[3*npt+i] + g_RT[1*npt+i]) - (g_RT[6*npt+i]));
    Kc[9][i] = exp((g_RT[6*npt+i] + g_RT[3*npt+i]) - (g_RT[i] + g_RT[1*npt+i]));
    Kc[10][i] = exp((g_RT[6*npt+i] + g_RT[3*npt+i]) - (g_RT[5*npt+i] + g_RT[5*npt+i]));
    Kc[11][i] = exp((g_RT[6*npt+i] + g_RT[4*npt+i]) - (g_RT[1*npt+i] + g_RT[5*npt+i]));
    Kc[12][i] = exp((g_RT[6*npt+i] + g_RT[5*npt+i]) - (g_RT[2*npt+i] + g_RT[1*npt+i]));
    Kc[13][i] = exp((g_RT[6*npt+i] + g_RT[6*npt+i]) - (g_RT[7*npt+i] + g_RT[1*npt+i]));
    Kc[14][i] = exp((g_RT[6*npt+i] + g_RT[6*npt+i]) - (g_RT[7*npt+i] + g_RT[1*npt+i]));
    Kc[15][i] = refC * exp((g_RT[7*npt+i]) - (g_RT[5*npt+i] + g_RT[5*npt+i]));
    Kc[16][i] = exp((g_RT[7*npt+i] + g_RT[3*npt+i]) - (g_RT[2*npt+i] + g_RT[5*npt+i]));
    Kc[17][i] = exp((g_RT[7*npt+i] + g_RT[3*npt+i]) - (g_RT[6*npt+i] + g_RT[i]));
    Kc[18][i] = exp((g_RT[7*npt+i] + g_RT[4*npt+i]) - (g_RT[5*npt+i] + g_RT[6*npt+i]));
    Kc[19][i] = exp((g_RT[7*npt+i] + g_RT[5*npt+i]) - (g_RT[6*npt+i] + g_RT[2*npt+i]));
    Kc[20][i] = exp((g_RT[7*npt+i] + g_RT[5*npt+i]) - (g_RT[6*npt+i] + g_RT[2*npt+i]));
  }
  ...
  for (int i=0; i<npt; i++) {
    double qdot, q_f, q_r, phi_f, phi_r, k_r;
    double alpha, redP, F, logPred, logFcent;
    double troe_c, troe_n, troe, F_troe;

    /*reaction 1: H + O2 <=> O + OH */
    phi_f = c[3*npt+i]*c[1*npt+i];
    q_f = phi_f * k_f[0][i];
    phi_r = c[4*npt+i]*c[5*npt+i];
    k_r = k_f[0][i] / Kc[0][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[1*npt+i] -= 1.0 * qdot;
    wdot[3*npt+i] -= 1.0 * qdot;
    wdot[4*npt+i] += 1.0 * qdot;
    wdot[5*npt+i] += 1.0 * qdot;

    /*reaction 2: O + H2 <=> H + OH */
    phi_f = c[4*npt+i]*c[0*npt+i];
    q_f = phi_f * k_f[1][i];
    phi_r = c[3*npt+i]*c[5*npt+i];
    k_r = k_f[1][i] / Kc[1][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    ...
    /*reaction 3: H2 + OH <=> H2O + H */
    phi_f = c[0*npt+i]*c[5*npt+i];
    q_f = phi_f * k_f[2][i];
    phi_r = c[2*npt+i]*c[3*npt+i];
    k_r = k_f[2][i] / Kc[2][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[0*npt+i] -= 1.0 * qdot;
    wdot[2*npt+i] += 1.0 * qdot;
    wdot[3*npt+i] += 1.0 * qdot;
    wdot[5*npt+i] -= 1.0 * qdot;

    /*reaction 4: O + H2O <=> OH + OH */
    phi_f = c[4*npt+i]*c[2*npt+i];
    q_f = phi_f * k_f[3][i];
    phi_r = c[5*npt+i]*c[5*npt+i];
    k_r = k_f[3][i] / Kc[3][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[2*npt+i] -= 1.0 * qdot;
    ...
    /*reaction 5: H2 + M <=> H + H + M */
    phi_f = c[0*npt+i];
    alpha = mixture[i] + 1.5*c[0*npt+i] + 11.*c[2*npt+i];
    q_f = phi_f * k_f[4][i];
    phi_r = c[3*npt+i]*c[3*npt+i];
    k_r = k_f[4][i] / Kc[4][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[0*npt+i] -= 1.0 * qdot;
    ...
    /*reaction 9: H + O2 (+M) <=> HO2 (+M) */
    phi_f = c[3*npt+i]*c[1*npt+i];
    alpha = mixture[i] + c[0*npt+i] + 10*c[2*npt+i] + -0.22*c[1*npt+i];
    redP = 1.e-12 * alpha / k_f[8][i] * 6.366e+20*exp(-1.72*tc[i]-264.08810621431689469*invT[i]);
    F = redP / (1. + redP);
    logPred = log10(redP);
    logFcent = log10((0.2*exp(T[i]/-1.e-30))+ (0.8*exp(T[i]/-1.e+30)));
    troe_c = -.4 - .67 * logFcent;
    troe_n = .75 - 1.27 * logFcent;
    troe = (troe_c + logPred) / (troe_n - .14*(troe_c + logPred));
    F_troe = pow(10., logFcent / (1.0 + troe*troe));
    ...
    /*reaction 10: HO2 + H <=> H2 + O2 */
    phi_f = c[6*npt+i]*c[3*npt+i];
    q_f = phi_f * k_f[9][i];
    phi_r = c[0*npt+i]*c[1*npt+i];
    k_r = k_f[9][i] / Kc[9][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[0*npt+i] += 1.0 * qdot;
    ...
    /*reaction 12: HO2 + O <=> O2 + OH */
    phi_f = c[6*npt+i]*c[4*npt+i];
    q_f = phi_f * k_f[11][i];
    phi_r = c[1*npt+i]*c[5*npt+i];
    k_r = k_f[11][i] / Kc[11][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[1*npt+i] += 1.0 * qdot;
    wdot[4*npt+i] -= 1.0 * qdot;
    ...
    /*reaction 13: HO2 + OH <=> H2O + O2 */
    phi_f = c[6*npt+i]*c[5*npt+i];
    q_f = phi_f * k_f[12][i];
    phi_r = c[2*npt+i]*c[1*npt+i];
    k_r = k_f[12][i] / Kc[12][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[1*npt+i] += 1.0 * qdot;
    wdot[2*npt+i] += 1.0 * qdot;
    ...
    /*reaction 14: HO2 + HO2 <=> H2O2 + O2 */
    phi_f = c[6*npt+i]*c[6*npt+i];
    q_f = phi_f * k_f[13][i];
    phi_r = c[7*npt+i]*c[1*npt+i];
    k_r = k_f[13][i] / Kc[13][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[1*npt+i] += 1.0 * qdot;
    wdot[6*npt+i] -= 1.0 * qdot;
    ...
    phi_f = c[7*npt+i];
    alpha = mixture[i] + 1.5*c[0*npt+i] + 11.*c[2*npt+i];
    redP = 1.e-6 * alpha / k_f[15][i] * 1.202e+17*exp(-22896.358294114746968*invT[i]);
    F = redP / (1. + redP);
    logPred = log10(redP);
    logFcent = log10((0.5*exp(T[i]/-1.e-30))+ (0.5*exp(T[i]/-1.e+30)));
    troe_c = -.4 - .67 * logFcent;
    troe_n = .75 - 1.27 * logFcent;
    troe = (troe_c + logPred) / (troe_n - .14*(troe_c + logPred));
    F_troe = pow(10., logFcent / (1.0 + troe*troe));
    ...
    /*reaction 17: H2O2 + H <=> H2O + OH */
    phi_f = c[7*npt+i]*c[3*npt+i];
    q_f = phi_f * k_f[16][i];
    phi_r = c[2*npt+i]*c[5*npt+i];
    k_r = k_f[16][i] / Kc[16][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[2*npt+i] += 1.0 * qdot;
    wdot[3*npt+i] -= 1.0 * qdot;
    ...
    phi_f = c[7*npt+i]*c[3*npt+i];
    q_f = phi_f * k_f[17][i];
    phi_r = c[6*npt+i]*c[0*npt+i];
    k_r = k_f[17][i] / Kc[17][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[0*npt+i] += 1.0 * qdot;
    wdot[3*npt+i] -= 1.0 * qdot;
    ...
    /*reaction 19: H2O2 + O <=> OH + HO2 */
    phi_f = c[7*npt+i]*c[4*npt+i];
    q_f = phi_f * k_f[18][i];
    phi_r = c[5*npt+i]*c[6*npt+i];
    k_r = k_f[18][i] / Kc[18][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[4*npt+i] -= 1.0 * qdot;
    wdot[5*npt+i] += 1.0 * qdot;
    ...
    /*reaction 20: H2O2 + OH <=> HO2 + H2O */
    phi_f = c[7*npt+i]*c[5*npt+i];
    q_f = phi_f * k_f[19][i];
    phi_r = c[6*npt+i]*c[2*npt+i];
    k_r = k_f[19][i] / Kc[19][i];
    q_r = phi_r * k_r;
    qdot = q_f - q_r;
    wdot[2*npt+i] += 1.0 * qdot;
    wdot[5*npt+i] -= 1.0 * qdot;
