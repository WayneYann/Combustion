...
if run:
    ...
    for name, cfl in runs:
        ...
        autorun('cflconv/sdc9_cfl%s' % name, 
                probin, { 'cfl': cfl, 'method': 2 },
                sdc,    { 'nnodes': 9, 'tol_residual': 1e-8 })
...
plt.loglog(cfl, rho_errors['sdc9'], '-^r', label='SDC9')
plt.legend(loc='best')
plt.xlabel('cfl')
...
plt.loglog(cfl, energy_errors['sdc9'], '-^r', label='SDC9')
plt.xlabel('cfl')
plt.ylabel('max abs error')
plt.title('energy')
...
# x = [ cfl[0], cfl[-1] ]
# y = [ e0, e0*(x[1]/x[0])**8 ]
# plt.loglog(x, y, '--r', label='16th order')
