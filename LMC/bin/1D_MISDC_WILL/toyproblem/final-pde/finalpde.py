...
def solve_pde(a, eps, r, Nx=300, dt=None, FT=0.05, max_iter=2):
   ...
   def advance(n):
       ...
       for k in range(max_iter):
           ...
           if k==0:
              guess = rk4(f, 0, yn, dt1)
           else:
              guess = y_prev1
           
           for j in range(Nx-2):
              y_curr1[j] = newton(lambda x: x - yn[j] - dt1*(FR(x) + f[j]),
                                  guess[j],
                                  fprime=lambda x: 1 - dt1*FRp(x),
                                  tol=10e-14)
           ...
           if k==0:
              guess = rk4(f, dt1, y_curr1, dt2)
           else:
              guess = y_prev2
           for j in range(len(f)):
              y_curr2[j] = newton(lambda x: x - y_curr1[j] - dt2*(FR(x) + f[j]), 
                                  guess[j],
                                  fprime=lambda x: 1 - dt2*FRp(x),
                                  tol=10e-14)
