...
def solve_it(a, d, r, dt, max_iter=10, method=0, plot_it=False):
    # final time
    #T = 3
    ...
    for n in range(N):
        ...
        for i in range(max_iter):
            ...
            f_const = d*y_AD - d*y_prev - r*0.5# - r*y_prev
            # piecewise linear or constant?
            if method==1:
                f_lin_prev = A_n + D_n# + r*y[n]
                f_lin_next = A + D# + r*y_prev
            else:
                avg = 0.5*(A_n + D_n + A + D)# + 0.5*(r*y[n] + r*y_prev)
                f_lin_prev = avg
                f_lin_next = avg
            ...
            if method == 2:
                y_next = (y[n] + dt*(d*y_AD - d*y_prev - r*y_prev) 
                               + (a + d + r)*0.5*dt*(y_prev + y[n])
                               - r*0.5*dt)/(1-r*dt)
                I_R = r*0.5*dt*(y_next + y[n]) - r*0.5*dt
            else:
                # solve the ODE here
    ...
    exact = (np.exp(C*t)*(2*a + 2*d + r) + r)/(2*C)
