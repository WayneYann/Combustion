...
c          -7 mean initial function evaluation failed
C
...
C KFLAG = -1.  Error test failed repeatedly or with ABS(H) = HMIN. -----
 530  MSG = 'DVODE--  At T(=R1) and step size H(=R2), the error'
      ...
      ISTATE = -4
      GO TO 560
C Error in intial call to F ----
 545  MSG = 'DVODE--  Initial function evaulations fail'
...
C
 700  CONTINUE
      ISTATE = -3
      RETURN
 750  MSG='DVODE--  Trouble from F.  ITASK = I1, TOUT = R1.       '
      CALL XERRWD (MSG, 60, 27, 1, 1, ITASK, 0, 1, TOUT, ZERO)
      ISTATE = -7
      RETURN
C
