...
ABecLaplacian::compFlux (D_DECL(MultiFab &xflux, MultiFab &yflux, MultiFab &zflux),
			 ...
			 int src_comp, int dst_comp)
{
    compFlux(D_DECL(xflux, yflux, zflux), in, bc_mode, true, src_comp, dst_comp);
}
...
void
ABecLaplacian::Write(std::string& outfile) const 
{
    // Set filenames
    std::string ABecLaplacianNAME = outfile + sep + "MG_Level_";
    int mgLevels = numLevels();
    Array<std::string> ABecLaplacianNAMElev(mgLevels);

    for (int i=0;i<mgLevels; ++i)
    {
        ABecLaplacianNAMElev[i] = BoxLib::Concatenate(ABecLaplacianNAME,i,2);
    }
    ...
    if (ParallelDescriptor::IOProcessor()) {
        ...
        std::ofstream ofs;
        if (verbose)
            cout << "ABecLaplacian::Write " << ": writing Header" << endl;
        ofs.open(hdrName.c_str());
        ofs << BL_SPACEDIM << '\n';
        ofs << geomarray[0].Domain() << '\n';
        ofs << gbox[0] << '\n';
        int amr_ratio = 2;
        ofs << amr_ratio << '\n';
        ofs << maxorder << '\n';
    ...
    for (int i=0;i<mgLevels; ++i)
    {
        if (verbose && ParallelDescriptor::IOProcessor())
            cout << "ABecLaplacian::Write: writing mg_level " << i << " to " << ABecLaplacianNAMElev[i] << endl;
        WriteSub(ABecLaplacianNAMElev[i],i);
    }

#if 0
    std::string DDBndryTNAME = outfile + sep + "DDBndryT";
    if (verbose && ParallelDescriptor::IOProcessor())
        cout << "ABecLaplacian::Write: writing DDBndryT to " << DDBndryTNAME << endl;
    Tbd->Write(DDBndryTNAME);

    std::string DDBndryYNAME = outfile + sep + "DDBndryY";
    if (verbose && ParallelDescriptor::IOProcessor())
        cout << "ABecLaplacian::Write: writing DDBndryY to " << DDBndryYNAME << endl;
    Ybd->Write(DDBndryYNAME);
#endif
}

void
ABecLaplacian::WriteSub(std::string& outfile, int level) const 
{
    bool verbose = true;
    if (verbose && ParallelDescriptor::IOProcessor()) {
        if (!BoxLib::UtilCreateDirectory(outfile, 0755))
        ...
        std::ofstream ofs;
        if (verbose)
            cout << "ABecLaplacian::WriteSub level " << level << ": writing Header" << endl;
        ofs.open(hdrName.c_str());
        ofs << gbox[level] << '\n';
        ofs.close();
    }
    ...
    if (acoefs[level]) {
        if (verbose && ParallelDescriptor::IOProcessor())
            cout << "ABecLaplacian::WriteSub level " << level << ": writing acoefs " << endl;
        VisMF::Write(*acoefs[level],outfile+sep+"acoefs");
    }
        
    for (int i=0; i<BL_SPACEDIM; ++i) {
        std::string dim = BoxLib::Concatenate("", i, 1);
        if (verbose && ParallelDescriptor::IOProcessor())
            cout << "ABecLaplacian::WriteSub level " << level << ": writing bcoefs " << i << endl;
        VisMF::Write(*bcoefs[level][i],outfile+sep+"bcoefs_"+dim);
    }
    cout << "ABecLaplacian::WriteSub level " << level << ": writing stencil weights " << endl;
    std::ofstream sos;
    sos.open(std::string(outfile+sep+"stencilWeight_grids").c_str());
    undrrelxr[level]->write(outfile+sep+"stencilWeight_",sos);
    sos.close();
}
