...
void
NavierStokes::ns_print_values (const char* message,
                                       ...
                                       int  last_scalar)
{
    ...
    int idx = 3;
    int jdx = 127;
    if (debug) ns_print_values ("S_new at 1 in scalar_advection_update", idx, jdx, 0, NUM_STATE, &S_new);
        ...
        for (MFIter Rho_mfi(rho_halftime); Rho_mfi.isValid(); ++Rho_mfi)
        {
            ...
            for (int sigma = sComp; sigma <= last_scalar; sigma++)
            {
			 ...
			 rho_halftime[Rho_mfi]);
                godunov->Add_aofs_tf(S_old[Rho_mfi],
				     S_new[Rho_mfi],
				     sigma,
				     1,
                                     Aofs[Rho_mfi],
    ...
    if (debug) ns_print_values ("S_new at 2 in scalar_advection_update", idx, jdx, 0, NUM_STATE, &S_new);
...
#endif
    if (debug) ns_print_values ("S_new at 3 in scalar_advection_update", idx, jdx, 0, NUM_STATE, &S_new);
}
...
NavierStokes::reflux ()
{
    ...
    {
	ns_print_values ("Ssync before Reflux", icoord, jcoord, Ssync);
    }
    fr_visc.Reflux(*Ssync,volume,scale,BL_SPACEDIM,0,NUM_STATE-BL_SPACEDIM,geom);
    if (debug && ParallelDescriptor::IOProcessor())
    {
	ns_print_values ("Ssync after Reflux", icoord, jcoord, Ssync);
    }
