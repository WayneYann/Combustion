...
ChemDriver::solveTransient_sdc(FArrayBox&        Snew,
			       ...
			       int               sdc_flag) const
{
  BL_ASSERT(Snew.box().contains(box) && Sold.box().contains(box));
...
//     std::cout<<"sCompT = "<<sCompT<<std::endl;
    ...
    if (sdc_flag == 0)
    {
        FORT_CONPSOLV_SDC(box.loVect(), box.hiVect(),
			  Snew.dataPtr(sCompRho), ARLIM(Snew.loVect()),
			                          ARLIM(Snew.hiVect()),
			  Snew.dataPtr(sCompY), ARLIM(Snew.loVect()),
			                        ARLIM(Snew.hiVect()),
			  Snew.dataPtr(sCompT), ARLIM(Snew.loVect()), 
			                        ARLIM(Snew.hiVect()),
			  Snew.dataPtr(sCompH), ARLIM(Snew.loVect()), 
			                        ARLIM(Snew.hiVect()),
			  Sold.dataPtr(sCompY), ARLIM(Sold.loVect()), 
			                        ARLIM(Sold.hiVect()),
			  Sold.dataPtr(sCompT), ARLIM(Sold.loVect()), 
			                        ARLIM(Sold.hiVect()),
