...
void productionRate(double * wdot, double * sc, double T)
{
    ...
    /*reaction 1: H + O2 <=> O + OH */
    phi_f = sc[3]*sc[1];
    k_f = 1e-06 * 3.547e+15*exp(-0.406*tc[0]-8352.8934356925419706/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[4]*sc[5];
    ...
    /*reaction 2: O + H2 <=> H + OH */
    phi_f = sc[4]*sc[0];
    k_f = 1e-06 * 50800*exp(2.67*tc[0]-3165.2328279116868543/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[3]*sc[5];
    ...
    /*reaction 4: O + H2O <=> OH + OH */
    phi_f = sc[4]*sc[2];
    k_f = 1e-06 * 2.97e+06*exp(2.02*tc[0]-6743.1033217832446098/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[5]*sc[5];
    ...
    k_f = 1e-06 * alpha * 4.577e+19*exp(-1.4*tc[0]-52525.75557669664704/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[3]*sc[3];
    ...
    redP = 1e-12 * alpha / k_f * 6.366e+20*exp(-1.72*tc[0]-264.08810621431689469/tc[1]);
    F = redP / (1 + redP);
    logPred = log10(redP);
    ...
    /*reaction 10: HO2 + H <=> H2 + O2 */
    phi_f = sc[6]*sc[3];
    k_f = 1e-06 * 1.66e+13*exp(-414.14731595728432012/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[0]*sc[1];
    ...
    /*reaction 11: HO2 + H <=> OH + OH */
    phi_f = sc[6]*sc[3];
    k_f = 1e-06 * 7.079e+13*exp(-148.44891641239232172/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[5]*sc[5];
    ...
    /*reaction 13: HO2 + OH <=> H2O + O2 */
    phi_f = sc[6]*sc[5];
    k_f = 1e-06 * 2.89e+13*exp(+250.09868290494571852/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[2]*sc[1];
    ...
    /*reaction 15: HO2 + HO2 <=> H2O2 + O2 */
    phi_f = sc[6]*sc[6];
    k_f = 1e-06 * 1.3e+11*exp(+819.89091359562974048/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[7]*sc[1];
    Kc = exp((g_RT[6] + g_RT[6]) - (g_RT[7] + g_RT[1]));
    k_r = k_f / Kc;
    ...
    phi_f = sc[7];
    alpha = mixture + 1.5*sc[0] + 11*sc[2];
    k_f = 1 * 2.951e+14*exp(-24370.783124922574643/tc[1]);
    redP = 1e-6 * alpha / k_f * 1.202e+17*exp(-22896.358294114746968/tc[1]);
    F = redP / (1 + redP);
    logPred = log10(redP);
    ...
    /*reaction 17: H2O2 + H <=> H2O + OH */
    phi_f = sc[7]*sc[3];
    k_f = 1e-06 * 2.41e+13*exp(-1997.7701632447369775/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[2]*sc[5];
    ...
    /*reaction 18: H2O2 + H <=> HO2 + H2 */
    phi_f = sc[7]*sc[3];
    k_f = 1e-06 * 4.82e+13*exp(-4000.5724931475210724/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[6]*sc[0];
    ...
    /*reaction 21: H2O2 + OH <=> HO2 + H2O */
    phi_f = sc[7]*sc[5];
    k_f = 1e-06 * 5.8e+14*exp(-4809.2416750957054319/tc[1]);
    q_f = phi_f * k_f;
    phi_r = sc[6]*sc[2];
    Kc = exp((g_RT[7] + g_RT[5]) - (g_RT[6] + g_RT[2]));
    k_r = k_f / Kc;
