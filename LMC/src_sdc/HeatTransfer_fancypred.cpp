                                             ...
                                             Real      theta_enthalpy)
{
  ...
  if (theta == 0) {  // fully explicit update, assume all RHS terms in Force, zero np1 flux-based terms
    ...
    if (do_reflux && updateFluxReg) {
      for (int d = 0; d < BL_SPACEDIM; d++) {
        for (MFIter fmfi(*SpecDiffusionFluxn[d]); fmfi.isValid(); ++fmfi) {
          if (level > 0) {
            if (is_predictor) { // we only get here if sdc_iterMAX=0
    ...
    if (theta_enthalpy > 0) {
      // update species with corrected fluxes, for species, snew = sold + dt*(theta*Dnp1 + F)
      ...
      //         NOTE: we only get here if sdc_iterMAX=0
      if (do_reflux && updateFluxReg) {
        for (int d = 0; d < BL_SPACEDIM; d++) {
          for (MFIter fmfi(*SpecDiffusionFluxn[d]); fmfi.isValid(); ++fmfi) {
            if (level > 0) {  // we only get here if sdc_iterMAX=0
		       ...
		       int  ncycle)
{
    ...
    showMF("sdc",Forcing,"sdc_F_for_D",level,parent->levelSteps(level));
...
HeatTransfer::differential_spec_diffuse_sync (Real dt)
{
  ...
  // Also, Ssync for species should contain rho^{n+1} * (delta Y)^sync
	...
	update.plus(Rhs[mfi],box,0,0,nspecies);
                         ...
                         MultiFab* u_mac)
{
    ...
    for (MFIter mfi(dpdt); mfi.isValid(); ++mfi)
    {
        ...
        dpdt[mfi].plus(-p_amb,grids[i]);
        dpdt[mfi].mult(1.0/dt,grids[i]);
        ...
        //  so dp/dt = (p0 - p)/dt.)
        //
        dpdt[mfi].minus(ugradp,grids[i],0,0,1);
        //
        // Make sure to divide by gamma *after* subtracting ugradp.
        //
        dpdt[mfi].divide(gamma[mfi],grids[i],0,0,1);

        if (dpdt_option == 0)
        {
	  dpdt[mfi].divide(S_old[mfi],grids[i],pComp,0,1);
        }
        else if (dpdt_option == 1)
        {
	  dpdt[mfi].divide(p_amb,grids[i],0,1);
        }
        else
        {
            const int ncomp = 1;
            ...
            dpdt[mfi].divide(p_denom,grids[i],0,0,1);
        }
        
        dpdt[mfi].mult(dpdt_factor,grids[i]);
    }
