...
void productionRate(double * wdot, double * sc, double T)
{
    ...
    static double logFcent_save[177];
    static double redP_save[177];
    ...
    if (T != T_save)
    {
      ...
      Kc_save[35] = irefC * exp((g_RT[1] + g_RT[3] + g_RT[30]) - (g_RT[6] + g_RT[30]));
      Kc_save[36] = irefC * exp((g_RT[1] + g_RT[3] + g_RT[31]) - (g_RT[6] + g_RT[31]));
      ...
      k_f_save[2] = 1e-06 * 50000*exp(2.67*tc[0]-3165.58*Tinv);
      ...
      k_f_save[11] = 1e-12 * 6.02e+14*exp(-1509.81*Tinv);
      ...
      k_f_save[20] = 1e-06 * 1.02e+07*exp(2*tc[0]-956.215*Tinv);
      k_f_save[21] = 1e-06 * 4.6e+19*exp(-1.41*tc[0]-14569.7*Tinv);
      k_f_save[22] = 1e-06 * 1.02e+07*exp(2*tc[0]-956.215*Tinv);
      k_f_save[23] = 1e-06 * 3e+13;
      k_f_save[24] = 1e-06 * 1.92e+07*exp(1.83*tc[0]-110.72*Tinv);
      ...
      k_f_save[37] = 1e-06 * 8.3e+13*exp(-7253.65*Tinv);
      ...
      k_f_save[44] = 1e-06 * 2.8e+13*exp(-537.494*Tinv);
      k_f_save[45] = 1e-06 * 1.34e+14*exp(-319.577*Tinv);
      ...
      k_f_save[51] = 1e-06 * 1.27e+16*exp(-0.63*tc[0]-192.753*Tinv);
      ...
      k_f_save[57] = 1e-06 * 2.3e+10*exp(1.05*tc[0]-1648.21*Tinv);
      ...
      k_f_save[73] = 1e-06 * 1.08e+12*exp(0.454*tc[0]-915.954*Tinv);
      ...
      k_f_save[86] = 1e-06 * 2.9e+13*exp(+251.636*Tinv);
      k_f_save[87] = 1e-06 * 1.75e+12*exp(-161.047*Tinv);
      k_f_save[88] = 1e-06 * 5.8e+14*exp(-4811.27*Tinv);
      ...
      k_f_save[131] = 1e-06 * 3.4e+12*exp(-347.257*Tinv);
      k_f_save[132] = 1e-06 * 9.46e+13*exp(+259.185*Tinv);
      k_f_save[133] = 1e-06 * 5e+13;
      k_f_save[134] = 1e-06 * 1.32e+13*exp(-754.907*Tinv);
      ...
      k_f_save[154] = 1e-06 * 2.675e+13*exp(-14494.2*Tinv);
      k_f_save[155] = 1e-06 * 3.6e+10*exp(-4499.25*Tinv);
      k_f_save[156] = 1e-06 * 24500*exp(2.47*tc[0]-2606.95*Tinv);
      k_f_save[157] = 1e-06 * 2.12e+16*exp(-0.97*tc[0]-312.028*Tinv);
      k_f_save[158] = 1e-06 * 4.99e+12*exp(0.1*tc[0]-5334.68*Tinv);
      ...
      k_f_save[165] = 1e-06 * 2.244e+18*exp(-1*tc[0]-8555.61*Tinv);
      k_f_save[166] = 1e-06 * 1.87e+17*exp(-1*tc[0]-8555.61*Tinv);
      k_f_save[167] = 1e-06 * 7.6e+12*exp(-201.309*Tinv);
      k_f_save[168] = 1e-06 * 1.8e+13*exp(-452.944*Tinv);
      k_f_save[169] = 1e-06 * 4.28e-13*exp(7.6*tc[0]+1776.55*Tinv);
      k_f_save[170] = 1e-06 * 5e+13*exp(-754.907*Tinv);
      k_f_save[171] = 1e-06 * 407000*exp(2.4*tc[0]-100.654*Tinv);
      k_f_save[172] = 1e-06 * 3.98e+12*exp(+120.785*Tinv);
      k_f_save[173] = 1 * 8e+12*exp(0.44*tc[0]-44675.4*Tinv);
      k_f_save[174] = 1e-06 * 8.4e+11*exp(-1950.18*Tinv);
      k_f_save[175] = 1e-06 * 1.6e+12*exp(-429.794*Tinv);
      k_f_save[176] = 1e-06 * 1e+13;

      logFcent_save[49] = log10((0.32*exp(T/-78))+ (0.68*exp(T/-1995))+ (exp(-5590*Tinv)));
      ...
      logFcent_save[58] = log10((0.2321*exp(T/-338))+ (0.7679*exp(T/-1812))+ (exp(-5081*Tinv)));
      logFcent_save[62] = log10((0.1098*exp(T/-144))+ (0.8902*exp(T/-2838))+ (exp(-45569*Tinv)));
      ...
      logFcent_save[94] = log10((0.7895*exp(T/-83.5))+ (0.2105*exp(T/-5398))+ (exp(-8370*Tinv)));
      ...
      logFcent_save[146] = log10((0.8493*exp(T/-134))+ (0.1507*exp(T/-2383))+ (exp(-7265*Tinv)));
      logFcent_save[157] = log10((0.4675*exp(T/-151))+ (0.5325*exp(T/-1038))+ (exp(-4970*Tinv)));
      logFcent_save[173] = log10((0.2655*exp(T/-180))+ (0.7345*exp(T/-1035))+ (exp(-5417*Tinv)));

      redP_save[49] = 1e-12 / k_f_save[49] * 3.2e+27*exp(-3.14*tc[0]-619.024*Tinv);
      redP_save[51] = 1e-12 / k_f_save[51] * 2.477e+33*exp(-4.76*tc[0]-1227.98*Tinv);
      redP_save[53] = 1e-12 / k_f_save[53] * 1.35e+24*exp(-2.57*tc[0]-717.162*Tinv);
      ...
      redP_save[58] = 1e-12 / k_f_save[58] * 3e+31*exp(-4.8*tc[0]-1660.8*Tinv);
      redP_save[62] = 1e-12 / k_f_save[62] * 8.6e+28*exp(-4*tc[0]-1522.4*Tinv);
      ...
      redP_save[73] = 1e-12 / k_f_save[73] * 1.2e+42*exp(-7.62*tc[0]-3507.8*Tinv);
      ...
      redP_save[94] = 1e-12 / k_f_save[94] * 2.7e+38*exp(-6.3*tc[0]-1560.14*Tinv);
      ...
      redP_save[146] = 1e-12 / k_f_save[146] * 2.7e+38*exp(-6.3*tc[0]-1560.14*Tinv);
      redP_save[157] = 1e-12 / k_f_save[157] * 1.77e+50*exp(-9.67*tc[0]-3130.35*Tinv);
      redP_save[173] = 1e-12 / k_f_save[173] * 7e+50*exp(-9.31*tc[0]-50256.7*Tinv);
    }
