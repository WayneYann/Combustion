...
Real RNS::gravy = 0.0;
Real RNS::gravz = 0.0;
//Real RNS::gravz = -980.665;

// Note: RNS::variableSetUp is in RNS_setup.cpp
...
RNS::read_params ()
{
    ...
    if ( Geometry::IsRZ() && (lo_bc[0] != Symmetry) ) {
        std::cerr << "ERROR:RNS::read_params: must set r=0 boundary condition to Symmetry for r-z\n";
        BoxLib::Error();
    }
    ...
    if ( Geometry::IsSPHERICAL() )
    {
      ...
      {
        std::cerr << "ERROR:RNS::read_params: must set r=0 boundary condition to Symmetry for spherical\n";
        BoxLib::Error();
      }
    }
#elif (BL_SPACEDIM == 2)
...
RNS::initData ()
{
    ...
    for (MFIter mfi(S_new); mfi.isValid(); ++mfi)
    {
        ...
        BL_FORT_PROC_CALL(RNS_INITDATA,cns_initdata)
...
RNS::estTimeStep (Real dt_old)
{
	...
	const Box& box = mfi.validbox();
	Real dt = estdt;
	BL_FORT_PROC_CALL(RNS_ESTDT,cns_estdt)
...
RNS::enforce_nonnegative_species (MultiFab& S_new)
{
    for (MFIter mfi(S_new); mfi.isValid(); ++mfi)
    {
       ...
       BL_FORT_PROC_CALL(RNS_ENFORCE_NONNEGATIVE_SPECIES,cns_enforce_nonnegative_species)
	 (BL_TO_FORTRAN(S_new[mfi]),bx.loVect(),bx.hiVect());
    }
}

void
RNS::enforce_consistent_e (MultiFab& S)
{
    for (MFIter mfi(S); mfi.isValid(); ++mfi)
    {
        ...
        const int* lo      = box.loVect();
        const int* hi      = box.hiVect();
        BL_FORT_PROC_CALL(RNS_ENFORCE_CONSISTENT_E,cns_enforce_consistent_e)
...
RNS::avgDown (int state_indx)
{
	...
	BL_FORT_PROC_CALL(RNS_AVGDOWN,cns_avgdown)
...
//
// Helper function for RNS::SyncInterp().
//
...
void
RNS::SyncInterp (MultiFab&      CrseSync,
                    int            c_lev,
                    MultiFab&      FineSync,
                    int            f_lev,
                    IntVect&       ratio,
                    ...
                    int            state_comp)
{
    ...
    default:
        BoxLib::Abort("RNS::SyncInterp(): how did this happen");
    }

    RNS&   fine_level = getLevel(f_lev);
...
void
RNS::reset_internal_energy(MultiFab& S_new)
{
  ...
  for (MFIter mfi(S_new); mfi.isValid(); ++mfi)
  {
      ...
      BL_FORT_PROC_CALL(RNS_RESET_INTERNAL_ENERGY,cns_reset_internal_energy)
...
void
RNS::computeTemp(MultiFab& State)
{
    reset_internal_energy(State);
    for (MFIter mfi(State); mfi.isValid(); ++mfi)
    {
        const Box bx = mfi.validbox();
	BL_FORT_PROC_CALL(RNS_COMPUTE_TEMP,cns_compute_temp)
