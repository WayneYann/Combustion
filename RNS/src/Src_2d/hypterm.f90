  ...
  subroutine hypterm(lo,hi,U,Ulo,Uhi,fx,fy)
    ...
    do i=tlo(1),thi(1)
       call reconstruct(tlo(2),thi(2), U(i,:,:), Ulo(2), Uhi(2), &
            UL=ULy(i,:,:), UR=URy(i,:,:), UG1=UG1y(i,:,:), UG2=UG2y(i,:,:), dir=2)
    end do
    ...
    do j=lo(2), hi(2)+1
       call reconstruct(lo(1),hi(1), ULy(:,j,:), lo(1)-3, hi(1)+3, &
            UG1=ULyG1(:,j,:), UG2=ULyG2(:,j,:), dir=1)
    end do

    do j=lo(2), hi(2)+1
       call reconstruct(lo(1),hi(1), URy(:,j,:), lo(1)-3, hi(1)+3, &
            UG1=URyG1(:,j,:), UG2=URyG2(:,j,:), dir=1)
    end do

    ! flux in y-direction for two Gauss points in x-direction
    do i=lo(1), hi(1)
       call riemann(lo(2), hi(2), ULyG1, URyG1, flux, dir=2)
       do n=1,NVAR
          do j=lo(2),hi(2)+1
       ...
       call riemann(lo(2), hi(2), ULyG2, URyG2, flux, dir=2)
       do n=1,NVAR
          do j=lo(2),hi(2)+1
             fy(i,j,n) = fy(i,j,n) + 0.5d0*flux(j,n)
          end do
    ...
    deallocate(ULy,URy, URyG1, ULyG2, ULyG1, URyG2, flux)
