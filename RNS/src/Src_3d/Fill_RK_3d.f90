...
subroutine rns_fill_rk4_bndry (lo, hi,  &
  ...
  !$omp parallel do collapse(3) private(i,j,k,n,Utt,Uttt)
  do n=1,NVAR
     do k=lo(3),hi(3)
     do j=lo(2),hi(2)
        ...
        if (stage .eq. 0) then
           ...
           do i=lo(1),hi(1)
              uu(i,j,k,n) = uu(i,j,k,n) + 0.5d0*dtdt*k1(i,j,k,n)
           end do
           ...
           if (stage .eq. 2) then
              ...
              do i=lo(1),hi(1)
                 uu(i,j,k,n) = uu(i,j,k,n) + 0.5d0*dtdt*k1(i,j,k,n) + 0.25d0*dtdt2*Utt(i) &
              ...
              do i=lo(1),hi(1)
                 uu(i,j,k,n) = uu(i,j,k,n) + dtdt*k1(i,j,k,n) + 0.5d0*dtdt2*Utt(i) &
