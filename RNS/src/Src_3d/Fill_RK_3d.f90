...
subroutine rns_fill_rk3_bndry (lo, hi,  &
  ...
  !$omp parallel do collapse(3) private(i,j,k,n,Ut)
  do n=1,NVAR
     do k=lo(3),hi(3)
     do j=lo(2),hi(2)

        do i=lo(1),hi(1)
           uu(i,j,k,n) = u0(i,j,k,n)+b1*k1(i,j,k,n)+b2*k2(i,j,k,n)+b3*k3(i,j,k,n)
        end do

        if (stage .eq. 0) cycle

        do i=lo(1),hi(1)
           Ut(i) = c1*k1(i,j,k,n)+c2*k2(i,j,k,n)+c3*k3(i,j,k,n)
        end do

        if (stage .eq. 1) then
           
           do i=lo(1),hi(1)
              uu(i,j,k,n) = uu(i,j,k,n) + dtdt*Ut(i)
           end do
           ...
           do i=lo(1),hi(1)
              uu(i,j,k,n) = uu(i,j,k,n) + 0.5d0*dtdt*Ut(i) &
                   + 0.25d0*dtdt2*(d1*k1(i,j,k,n)+d2*k2(i,j,k,n)+d3*k3(i,j,k,n))
           end do
