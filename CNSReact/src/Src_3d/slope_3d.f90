...
! ::: 
      ...
      subroutine pslope(p,rho,flatn,qd_l1,qd_l2,qd_l3,qd_h1,qd_h2,qd_h3, &
        ...
        double precision,intent(in )::  p  ( qd_l1: qd_h1, qd_l2: qd_h2, qd_l3: qd_h3)
        double precision,intent(in )::  rho( qd_l1: qd_h1, qd_l2: qd_h2, qd_l3: qd_h3)
        double precision,intent(in )::flatn( qd_l1: qd_h1, qd_l2: qd_h2, qd_l3: qd_h3)
        double precision,intent(out)::  dpx(qpd_l1:qpd_h1,qpd_l2:qpd_h2,qpd_l3:qpd_h3)
        double precision,intent(out)::  dpy(qpd_l1:qpd_h1,qpd_l2:qpd_h2,qpd_l3:qpd_h3)
        double precision,intent(out)::  dpz(qpd_l1:qpd_h1,qpd_l2:qpd_h2,qpd_l3:qpd_h3)
        ...
        if(iorder.eq.1) then
           ...
           do j = ilo2-1, ihi2+1
              ...
              do i = ilo1-2, ihi1+2
                 ...
                 dlft = dlft - 0.5d0 * &
                      (rho(i,j,k3d)+rho(i-1,j,k3d))*gravx*dx
                 drgt = drgt - 0.5d0 * &
                      (rho(i,j,k3d)+rho(i+1,j,k3d))*gravx*dx
              ...
              do i = ilo1-1, ihi1+1
                 ...
                 dpx(i,j,kc) = dpx(i,j,kc) + rho(i,j,k3d)*gravx*dx
              enddo
           enddo
           !$OMP END PARALLEL DO
           ...
           do i = ilo1-1, ihi1+1
              ...
              do j = ilo2-2, ihi2+2
                 ...
                 ! Subtract off (rho * grav) so as not to limit that part of the slope
                 dlft = dlft - 0.5d0 * &
                      (rho(i,j,k3d)+rho(i,j-1,k3d))*gravy*dy
                 drgt = drgt - 0.5d0 * &
                      (rho(i,j,k3d)+rho(i,j+1,k3d))*gravy*dy
              ...
              do j = ilo2-1, ihi2+1
                 ...
                 dpy(i,j,kc) = dpy(i,j,kc) + rho(i,j,k3d)*gravy*dy
              enddo
           enddo
           !$OMP END PARALLEL DO
           ...
           do j = ilo2-1, ihi2+1
              do i = ilo1-1, ihi1+1
                 ...
                 dm = dm - 0.5d0 * (rho(i,j,k)+rho(i,j,k-1))*gravz*dz
                 dp = dp - 0.5d0 * (rho(i,j,k)+rho(i,j,k+1))*gravz*dz
                 dc = 0.5d0*(dm+dp)
                 ds = sign( 1.d0, dc )
                 if (dm*dp .ge. 0.d0) then
                 ...
                 dpz(i,j,kc) = dpz(i,j,kc) + rho(i,j,k3d)*gravz*dz
              enddo
           enddo
           !$OMP END PARALLEL DO
