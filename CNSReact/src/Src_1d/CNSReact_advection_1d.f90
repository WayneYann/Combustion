...
! ::: 
      ...
      use meth_params_module, only : QVAR, QU, NVAR, NHYP, URHO, normalize_species
      ...
      double precision delta(1),dt,courno
...
!     The temperature is used as an initial guess for the eos call and will be overwritten
      do i = loq(1),hiq(1)
         ...
         if (uin(i,URHO) .le. 0.d0) then
            print *,'   '
            print *,'>>> Error: CNSReact_1d::ctoprim ',i
            print *,'>>> ... negative density ',uin(i,URHO)
            print *,'    '
            call bl_error("Error:: CNSReact_1d.f90 :: ctoprim")
         end if
...
!     Get gamc, p, T, c, csml using q state
      do i = loq(1), hiq(1)
         ...
         call eos_given_ReX(gamc(i), q(i,QPRES), c(i), q(i,QTEMP), dpdrho(i), dpde(i), &
                            q(i,QRHO), q(i,QREINT), q(i,QFS:), pt_index)
...
!     Make this "rho e" instead of "e"
      ...
      do i = lo(1)-1, hi(1)+1
         ...
         srcQ(i,QPRES  ) =    dpde(i) * srcQ(i,QREINT) / q(i,QRHO) &
         ...
         do iadv=1,nadv
            srcQ(i,QFA+iadv-1) = src(i,UFA+iadv-1)/q(i,QRHO)
         end do
...
!     Local variables
      ...
      if (ppm_type .ne. 0) then
        ...
        call bl_error("Error:: CNSReact_1d.f90 :: trace")
      end if
