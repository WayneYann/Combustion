         ...
         do j = loq(2),hiq(2)
            do i = loq(1),hiq(1)
               ...
               if (uin(i,j,k,URHO) .le. 0.d0) then
                  print *,'   '
                  print *,'>>> Error: CNSReact_3d::ctoprim ',i,j,k
      ...
      do k = lo(3)-1, hi(3)+1
         do j = lo(2)-1, hi(2)+1
            do i = lo(1)-1, hi(1)+1
               ...
               srcQ(i,j,k,QU    ) = src(i,j,k,UMX) / q(i,j,k,QRHO)
               srcQ(i,j,k,QV    ) = src(i,j,k,UMY) / q(i,j,k,QRHO)
               srcQ(i,j,k,QW    ) = src(i,j,k,UMZ) / q(i,j,k,QRHO)
               srcQ(i,j,k,QREINT) = src(i,j,k,UEDEN) - q(i,j,k,QU)*src(i,j,k,UMX) - &
                                                       q(i,j,k,QV)*src(i,j,k,UMY) - &
                                                       q(i,j,k,QW)*src(i,j,k,UMZ)
               srcQ(i,j,k,QPRES ) =   dpde(i,j,k) * srcQ(i,j,k,QREINT) / q(i,j,k,QRHO) &
         ...
         do j = lo(2),hi(2)
            do i = lo(1),hi(1)
               ...
               if (courx .gt. 1.d0) then
                  print *,'   '
                  call bl_warning("Warning:: CNSReact_3d.f90 :: CFL violation in ctoprim")
...
! ::: 
      ...
      if (ppm_type .ne. 0) then
        ...
        call bl_error("Error:: CNSReact_3d.f90 :: tracexy")
      end if
...
! ::: 
      ...
      if (ppm_type .ne. 0) then
        ...
        call bl_error("Error:: CNSReact_3d.f90 :: tracez")
      end if
...
! ::: 
      ...
      !$OMP PRIVATE(pnewr,pnewl,rhotmp,rhoekenr,rhoekenl)
      do j = jlo, jhi 
         do i = ilo, ihi 
            ...
            rhotmp = rrnewl
            qmo(i,j,kc,QRHO  ) = rhotmp        + hdt*srcQ(i,j,k3d,QRHO)
            qmo(i,j,kc,QU    ) = runewl/rhotmp + hdt*srcQ(i,j,k3d,QU) + hdt*grav(i,j,k3d-1,1)
            qmo(i,j,kc,QV    ) = rvnewl/rhotmp + hdt*srcQ(i,j,k3d,QV) + hdt*grav(i,j,k3d-1,2)
            qmo(i,j,kc,QW    ) = rwnewl/rhotmp + hdt*srcQ(i,j,k3d,QW) + hdt*grav(i,j,k3d-1,3)
            rhoekenl = 0.5d0*(runewl**2 + rvnewl**2 + rwnewl**2)/rhotmp
            qmo(i,j,kc,QREINT) = renewl - rhoekenl + hdt*srcQ(i,j,k3d,QREINT)
            qmo(i,j,kc,QPRES ) = pnewl         + hdt*srcQ(i,j,k3d,QPRES)
...
! ::: 
      ...
      !$OMP PRIVATE(rhotmp,rhoekenr,rhoekenl)
      do j = jlo, jhi 
         do i = ilo, ihi 
