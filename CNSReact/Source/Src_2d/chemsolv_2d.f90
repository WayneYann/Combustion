      subroutine chemsolv(lo, hi, u, u_l1, u_l2, u_h1, u_h2, dt, idbg)
      ...
      integer            :: i, j, n, MF, ISTATE, T_from_eY, Niter
      ...
      SAVE   /VHACK/
      double precision scale, ekin, u1, u2
!     integer is_bad
...
!                   (u(i,j,NTHERM+nadv+iC2H2).gt. 0.d0) .and. &
!                   (u(i,j,NTHERM+nadv+iO2  ).gt. 0.d0) ) then
...
!     Note: pass eint in NP slot, and density in rho slot
                  ...
                  sum = 0.d0
                  do n=1,Nspec
                     Ytemp(n) = MAX(u(i,j,NTHERM+NADV+n)/u(i,j,1),0.d0)
!                    Ytemp(n) = u(i,j,NTHERM+NADV+n)/u(i,j,1)
                     sum = sum+Ytemp(n)
                  end do
                  if (iN2 .gt. 0) then
...
!     Note: ensure that T initialized to a good value, load Y,T into IC
                  ...
                  if (ISTATE .LE. -1) then
                     ...
                     write(lout,996) 'T ', &             
                          u(i,j,NTHERM+1),u(i,j,NTHERM+1),RWRK(NZ),deriv(1)
                     do n=1,Nspec
                        call get_spec_name(name,n)
                        write(lout,996) name,u(i,j,NTHERM+NADV+n)/u(i,j,1), &
                             Ytemp(n), &
                             RWRK(NZ+n),deriv(n+1)
                     end do
