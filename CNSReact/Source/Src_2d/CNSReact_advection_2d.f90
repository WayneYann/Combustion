...
! ::: 
      ...
      use meth_params_module, only : URHO, QVAR, NVAR, NHYP, &
                                     normalize_species, allow_negative_energy
...
!     Conservative update
      ...
      deallocate(q,gamc,flatn,c,csml,div,pgdx,pgdy,pdivu)
      ...
      do j = loq(2),hiq(2)
         do i = loq(1),hiq(1)
            ...
            if (uin(i,j,URHO) .le. 0.d0) then
               print *,'   '
               print *,'>>> Error: CNSReact_2d::ctoprim ',i,j
               print *,'>>> ... negative density ',uin(i,j,URHO)
               print *,'    '
               call bl_error("Error:: CNSReact_2d.f90 :: ctoprim")
            end if
      ...
      do j = loq(2), hiq(2)
        do i = loq(1), hiq(1)
           ...
           call eos_given_ReX(gamc(i,j), q(i,j,QPRES), c(i,j), q(i,j,QTEMP), &
                              dpdrho(i,j), dpde(i,j), &
                              q(i,j,QRHO), q(i,j,QREINT), q(i,j,QFS:), pt_index)
...
!     Compute sources in terms of Q
      do j = lo(2)-1, hi(2)+1
        do i = lo(1)-1, hi(1)+1
           ...
           srcQ(i,j,QU    ) = src(i,j,UMX) / q(i,j,QRHO)  
           srcQ(i,j,QV    ) = src(i,j,UMY) / q(i,j,QRHO)
           srcQ(i,j,QREINT) = src(i,j,UEDEN) - q(i,j,QU) *src(i,j,UMX) - &
                                               q(i,j,QV) *src(i,j,UMY)
           srcQ(i,j,QPRES ) =   dpde(i,j) * srcQ(i,j,QREINT) / q(i,j,QRHO) &
      ...
      do j = lo(2),hi(2)
         do i = lo(1),hi(1)
            ...
            if (courx .gt. 1.d0) then
               print *,'   '
               call bl_warning("Warning:: CNSReact_2d.f90 :: CFL violation in ctoprim")
...
! ::: 
      ...
      if (ppm_type .ne. 0) then
        ...
        call bl_error("Error:: CNSReact_2d.f90 :: trace")
      end if
